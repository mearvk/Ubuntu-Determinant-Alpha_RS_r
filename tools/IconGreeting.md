...
The xmcLook icon set is presented here as the public-facing visual greeting for the tools collection. Each icon should be understood as a member of one system rather than as an isolated graphic.
...
This document is the visual greeting and icon-gallery companion for the xmcLook tooling within `/tools`. It is intentionally written as a human-readable design document so the icon system can be understood before the implementation is opened.
