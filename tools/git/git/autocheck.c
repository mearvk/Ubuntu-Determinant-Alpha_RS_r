/* Native autocheck planning implementation. */
#include "git-compat-util.h"
#include "autocheck.h"

int git_autocheck_plan_make(struct git_autocheck_plan *plan,
	uintmax_t age_days, uintmax_t conflict_count,
	int local_changes, int remote_changes)
{
	if (!plan || age_days > UINTMAX_MAX || conflict_count > UINTMAX_MAX)
		return -1;
	...
	if (conflict_count) {
		if (git_autocheck_merge_preferred(age_days))
			plan->resolution = GIT_AUTOCHECK_MERGE;
		else
			plan->resolution = GIT_AUTOCHECK_REBASE;
