...
def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Korea archive builder/1.2'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        ...
        lines += ['', '## Source notes', '- KOSIS, MAFRA/KASS and KAMIS are preferred Korean primary statistical families for table-level replacement or supplementation where an equivalent series is available.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations.', '- World Bank WDI supplies reproducible macroeconomic, land, employment and machinery series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/korea/build_korea_annual_data.py`.']
