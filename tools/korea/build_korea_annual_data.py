...
annual observation. Korean primary statistical families are preferred where an equivalent
machine-readable series is available; international series provide reproducible fallback.
"""
...
def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Korea archive builder/1.1'})
...
spec = {
    ...
    'potatoes area harvested': find_fao(['potatoes'], 'Area harvested'),
    ...
    'potatoes yield': find_fao(['potatoes'], 'Yield'),
}
...
for y in YEARS:
    for fn in ['ECONOMY', 'AGRICULTURE', 'PRODUCE', 'FARMING_STATS', 'GRAINS', 'MEAT', 'LIVESTOCK', 'VEGETABLES', 'FRUIT', 'WATER']:
        lines = [
            f'# Korea {y} — {fn}', '',
            f'- **Year:** {y}',
            '- **Geography:** Republic of Korea, national aggregate',
        ...
        if fn == 'ECONOMY':
            ...
            add('Agricultural tractors', series['Agricultural machinery tractors'].get(y), 'tractors', 'World Bank WDI')
            ...
            for n in ('soybean production t', 'vegetables production t', 'fruit production t'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('rice area harvested', 'wheat area harvested', 'barley area harvested', 'maize area harvested', 'potatoes area harvested'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('rice yield', 'wheat yield', 'barley yield', 'maize yield', 'potatoes yield'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('cattle stock', 'pigs stock', 'sheep stock', 'goats stock'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'PRODUCE':
            for n in ('soybean production t', 'cotton production t', 'sugar cane production t', 'oilcrops production t', 'vegetables production t', 'fruit production t'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            ...
            add('Wheat production', series['Wheat production t'].get(y), 'tonnes', 'FAO via Our World in Data')
            add('Maize production', series['Maize production t'].get(y), 'tonnes', 'FAO via Our World in Data')
            for n in ('rice area harvested', 'wheat area harvested', 'barley area harvested', 'maize area harvested'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('rice yield', 'wheat yield', 'barley yield', 'maize yield'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'MEAT':
            ...
            for n in ('cow milk production t', 'eggs production t'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            vals, item, unit = spec['pigs slaughtered']; add('Pigs slaughtered', vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'VEGETABLES':
        ...
        lines += [
            '', '## Source notes',
            '- Korean primary statistical families (KOSIS, MAFRA/KASS and KAMIS) are preferred for table-level replacement or supplementation where an equivalent series is available.',
            '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations.',
            '- World Bank WDI supplies reproducible macroeconomic, land, employment and machinery series.',
            ...
            '', 'Generated by `tools/korea/build_korea_annual_data.py`.'
        ]
