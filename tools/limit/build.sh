...
SRC="$ROOT/tools/limit/limit.c"
OUT="$ROOT/tools/limit/limit"
CC="${MEARVK_GCC:-}"
if [[ -z "$CC" ]]; then
  for candidate in \
...
META=( -fme-metadata -fme-metadata-edition=Ubuntu.Determinant.Beta.Restricted -fme-metadata-version=1.01 -fme-metadata-company=MEARVK -fme-metadata-source=tools/limit/limit.c )
