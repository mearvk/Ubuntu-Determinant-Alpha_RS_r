...
MEARVK checkout.
  ...
  --prepare   create tools/gcc/gcc-github-latest-import
...
ORIGIN_CLEAN=$(printf '%s' "$ORIGIN" |
    sed -E 's#https://[^@/]+@#https://#')

# The checkout can have an old/mispointed origin. Do not reject the local
# master clone merely because origin says Alpha.
#
# For --refresh, Beta is used explicitly below, so we cannot accidentally
# refresh from Alpha.
...
if [ "$REFRESH" -eq 1 ]; then
    ...
    echo "Refreshing from Beta explicitly:"
    echo "$MEARVK_URL"
    ...
    # Only fast-forward. Never silently merge or rewrite local work.
...
if [ -f "$SCRIPT_DIR/gcc-16.2.0/configure" ]; then
    ...
    echo "Expected:" >&2
    echo "  gcc-16.2.0/" >&2
    echo "  gcc-github-latest/" >&2
    echo "  gcc-github-latest-import/" >&2
    echo >&2
    echo "The GCC source must already have been pulled into this" >&2
    echo "MEARVK checkout." >&2
...
while IFS= read -r -d '' file; do
    ...
    if [ "$SIZE" -ge "$MAX_BYTES" ]; then
        ...
        if ! grep -Fq "$(printf '\t%s' "$REL")" \
            "$LARGE_LIST" 2>/dev/null
        then
            printf '%s\t%s\n' "$SIZE" "$REL"
        fi
    fi
...
TRACKED_COUNT=$(
    git -C "$REPO_ROOT" ls-files "tools/gcc" |
    wc -l |
    tr -d ' '
)

STATUS=$(
    git -C "$REPO_ROOT" status --short -- "tools/gcc"
)
...
if [ "$PREPARE" -eq 1 ]; then
    ...
    echo
    echo "Prepared source-only copy:"
    echo "$IMPORT_DIR"
