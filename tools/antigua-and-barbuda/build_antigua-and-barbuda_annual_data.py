#!/usr/bin/env python3
"""Build Antigua and Barbuda annual statistical markdown records (1965-2026).
...
annual observation. Statistics Division is the preferred national primary family for table-level
supplementation where a compatible machine-readable series exists; international series
(FAOSTAT, World Bank WDI, AQUASTAT via Our World in Data) provide the reproducible baseline.

Mirrors tools/afghanistan/build_afghanistan_annual_data.py. Designed to run where outbound
network access to the source APIs exists (e.g. GitHub Actions). In a restricted environment
every value resolves to N/A, a valid non-fabricated state.
"""
from pathlib import Path
import csv, io, json, zipfile, urllib.request

ROOT = Path('national/antigua-and-barbuda')
...
WB = 'https://api.worldbank.org/v2/country/ATG/indicator/{indicator}?format=json&per_page=100'
ENTITIES = {'Antigua and Barbuda'}

def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Antigua and Barbuda archive builder/1.0'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        lines = [f'# Antigua and Barbuda {y} \u2014 {fn}', '', f'- **Year:** {y}', '- **Geography:** Antigua and Barbuda, national aggregate', '- **Method:** source observations only; no interpolation or invented annual totals.', '- **Metadata:** see `./META.md` and `../META.md`.']
        if y == 2026:
        ...
        if fn == 'ECONOMY':
            ...
            add('Potato production', series['Potato production t'].get(y), 'tonnes', 'FAO via Our World in Data')
            for n in ('wheat area harvested','maize area harvested','barley area harvested','rice area harvested'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('wheat yield','maize yield','barley yield','rice yield'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('cattle stock','sheep stock','goats stock','pigs stock'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'PRODUCE':
            for n in ('soybean production t','grapes production t','vegetables production t','fruit production t'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'GRAINS':
            add('Cereal production', series['Cereal production t'].get(y), 'tonnes', 'FAO via Our World in Data')
            add('Wheat production', series['Wheat production t'].get(y), 'tonnes', 'FAO via Our World in Data')
            ...
            for n in ('wheat area harvested','maize area harvested','barley area harvested'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
            for n in ('wheat yield','maize yield','barley yield'):
                vals, item, unit = spec[n]; add(n, vals.get(y), unit, 'FAOSTAT QCL')
        elif fn == 'MEAT':
        ...
        lines += ['', '## Source notes', '- Statistics Division is the preferred national primary family for table-level replacement or supplementation where an equivalent series exists.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations (coverage from 1961).', '- World Bank WDI supplies reproducible macroeconomic, land and employment series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/antigua-and-barbuda/build_antigua-and-barbuda_annual_data.py`.']
