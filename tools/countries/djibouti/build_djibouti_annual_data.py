#!/usr/bin/env python3
"""Build Djibouti annual statistical markdown records (1965-2026).

Sources fetched at build time. No interpolation. Missing observations remain N/A; 2026 is
marked incomplete unless a source reports a final annual observation. INSTAD (Institut National de la Statistique de Djibouti) is the
...
WB = 'https://api.worldbank.org/v2/country/DJI/indicator/{indicator}?format=json&per_page=100'
ENTITIES = {'Djibouti'}

def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Djibouti archive builder/1.0'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        lines = [f'# Djibouti {y} \u2014 {fn}', '', f'- **Year:** {y}', '- **Geography:** Republic of Djibouti, national aggregate', '- **Method:** source observations only; no interpolation or invented annual totals.', '- **Metadata:** see `./META.md` and `../META.md`.']
        if y == 2026:
        ...
        lines += ['', '## Source notes', '- INSTAD (Institut National de la Statistique de Djibouti) is the preferred national primary family for table-level replacement or supplementation where an equivalent series exists.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations (coverage from 1961).', '- World Bank WDI supplies reproducible macroeconomic, land and employment series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/djibouti/build_djibouti_annual_data.py`.']
