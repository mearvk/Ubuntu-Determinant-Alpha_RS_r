#!/usr/bin/env python3
"""Build Armenia annual statistical markdown records (1965-2026).
...
annual observation. ARMSTAT (Statistical Committee of Armenia) is the preferred national primary family for table-level
...
WB = 'https://api.worldbank.org/v2/country/ARM/indicator/{indicator}?format=json&per_page=100'
ENTITIES = {'Armenia'}

def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Armenia archive builder/1.0'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        lines = [f'# Armenia {y} \u2014 {fn}', '', f'- **Year:** {y}', '- **Geography:** Republic of Armenia, national aggregate', '- **Method:** source observations only; no interpolation or invented annual totals.', '- **Metadata:** see `./META.md` and `../META.md`.']
        if y == 2026:
        ...
        lines += ['', '## Source notes', '- ARMSTAT (Statistical Committee of Armenia) is the preferred national primary family for table-level replacement or supplementation where an equivalent series exists.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations (coverage from 1961).', '- World Bank WDI supplies reproducible macroeconomic, land and employment series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/armenia/build_armenia_annual_data.py`.']
