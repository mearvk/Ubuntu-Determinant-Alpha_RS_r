#!/usr/bin/env python3
"""Build Andorra annual statistical markdown records (1965-2026).
...
annual observation. Departament d\'Estadistica is the preferred national primary family for table-level
...
WB = 'https://api.worldbank.org/v2/country/AND/indicator/{indicator}?format=json&per_page=100'
ENTITIES = {'Andorra'}

def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Andorra archive builder/1.0'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        lines = [f'# Andorra {y} \u2014 {fn}', '', f'- **Year:** {y}', '- **Geography:** Principality of Andorra, national aggregate', '- **Method:** source observations only; no interpolation or invented annual totals.', '- **Metadata:** see `./META.md` and `../META.md`.']
        if y == 2026:
        ...
        lines += ['', '## Source notes', '- Departament d\'Estadistica is the preferred national primary family for table-level replacement or supplementation where an equivalent series exists.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations (coverage from 1961).', '- World Bank WDI supplies reproducible macroeconomic, land and employment series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/andorra/build_andorra_annual_data.py`.']
