#!/usr/bin/env python3
"""Build Cuba annual statistical markdown records (1965-2026).

Sources fetched at build time. No interpolation. Missing observations remain N/A; 2026 is
marked incomplete unless a source reports a final annual observation. ONEI (Oficina Nacional de Estadistica e Informacion) is the
...
WB = 'https://api.worldbank.org/v2/country/CUB/indicator/{indicator}?format=json&per_page=100'
ENTITIES = {'Cuba'}

def get(url):
    req = urllib.request.Request(url, headers={'User-Agent': 'Ubuntu.Determinant.Beta.Restricted Cuba archive builder/1.0'})
...
for y in YEARS:
    for fn in ['ECONOMY','AGRICULTURE','PRODUCE','FARMING_STATS','GRAINS','MEAT','LIVESTOCK','VEGETABLES','FRUIT','WATER']:
        lines = [f'# Cuba {y} \u2014 {fn}', '', f'- **Year:** {y}', '- **Geography:** Republic of Cuba, national aggregate', '- **Method:** source observations only; no interpolation or invented annual totals.', '- **Metadata:** see `./META.md` and `../META.md`.']
        if y == 2026:
        ...
        lines += ['', '## Source notes', '- ONEI (Oficina Nacional de Estadistica e Informacion) is the preferred national primary family for table-level replacement or supplementation where an equivalent series exists.', '- FAOSTAT Crops and livestock products (QCL) supplies item-level crop/livestock production, area, yield and stock observations (coverage from 1961).', '- World Bank WDI supplies reproducible macroeconomic, land and employment series.', '- AQUASTAT freshwater withdrawals are represented through the World Bank/Our World in Data series.', '- Definitions, revisions, flags and coverage remain source-controlled; missing observations remain `N/A`.', '', 'Generated by `tools/cuba/build_cuba_annual_data.py`.']
