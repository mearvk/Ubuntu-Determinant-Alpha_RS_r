...
int main(int argc, char **argv)
{
    ...
    const char *java_entry = NULL;
    const char *xclass = NULL;
    const char *native_payload = NULL;
    const char *bootstrap = NULL;
    const char *icon = NULL;
    const char *icon_sha256 = NULL;
    FILE *out = NULL;
    uint64_t bs = 0;
    uint64_t icon_size = 0;
    uint64_t native_size = 0;
    uint64_t xclass_size = 0;
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--output") && i + 1 < argc)
            ...
            entry = argv[++i];
        else if (!strcmp(argv[i], "--java") && i + 1 < argc)
            java_entry = argv[++i];
        else if (!strcmp(argv[i], "--xclass") && i + 1 < argc)
            xclass = argv[++i];
        else if (!strcmp(argv[i], "--native") && i + 1 < argc)
            native_payload = argv[++i];
        else if (!strcmp(argv[i], "--bootstrap") && i + 1 < argc)
            bootstrap = argv[++i];
        else if (!strcmp(argv[i], "--icon") && i + 1 < argc)
            icon = argv[++i];
        else if (!strcmp(argv[i], "--icon-sha256") && i + 1 < argc)
            icon_sha256 = argv[++i];
        else {
            usage(argv[0]);
            return 2;
    ...
    if (copy_file(out, bootstrap, &bs) != 0)
        goto io_fail;
    ...
    int ml = snprintf(manifest, sizeof manifest,
                      ...
                      icon ? "embedded" : "none",
                      icon_sha256 ? icon_sha256 : "unavailable");

    if (ml < 0 || (size_t)ml >= sizeof manifest) {
        fprintf(stderr, "manifest too large\n");
        goto io_fail;
    }

    asysma_native_layout h;
    memset(&h, 0, sizeof h);
    memcpy(h.magic, ASYSMA_NATIVE_MAGIC, 8);
    h.version = ASYSMA_NATIVE_LAYOUT_VERSION;
    ...
    h.manifest_offset = (uint64_t)header_pos + sizeof h;
    h.manifest_size = (uint64_t)ml;

    if (fwrite(&h, 1, sizeof h, out) != sizeof h)
        goto io_fail;

    if (fwrite(manifest, 1, (size_t)ml, out) != (size_t)ml)
        goto io_fail;
    ...
    if (icon) {
        ...
        if (pos < 0)
            goto io_fail;
        h.icon_offset = (uint64_t)pos;
        if (copy_file(out, icon, &icon_size) != 0)
            goto io_fail;
    }
    ...
    if (native_payload) {
        ...
        if (pos < 0)
            goto io_fail;
        h.payload_offset = (uint64_t)pos;
        if (copy_file(out, native_payload, &native_size) != 0)
            goto io_fail;
    }

    if (xclass && copy_file(out, xclass, &xclass_size) != 0)
        goto io_fail;

    h.icon_size = icon_size;
    h.payload_size = native_size;

    if (fseek(out, header_pos, SEEK_SET) != 0 ||
        fwrite(&h, 1, sizeof h, out) != sizeof h)
        goto io_fail;

    fclose(out);
    printf("created %s (bootstrap %llu, native %llu, xclass %llu, icon %llu bytes)\n",
           out_path, (unsigned long long)bs,
           (unsigned long long)native_size,
           (unsigned long long)xclass_size,
           (unsigned long long)icon_size);
    return 0;
