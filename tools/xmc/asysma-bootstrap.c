...
static int find_layout(FILE *f, asysma_native_layout *layout, long *position) {
    ...
    for (;;) {
        ...
        for (size_t i = 0; i + sizeof *layout <= n; ++i) {
            ...
            asysma_native_layout candidate;
            memcpy(&candidate, block + i, sizeof candidate);
            if (candidate.version != ASYSMA_NATIVE_LAYOUT_VERSION ||
                candidate.header_size < sizeof candidate) continue;
            if (candidate.manifest_offset > candidate.payload_offset) continue;
            *layout = candidate;
            *position = base + (long)i;
            return 1;
        }
...
int main(void) {
    ...
    if (!n || n >= sizeof self) { fprintf(stderr, "ASYSMA bootstrap: cannot locate executable\n"); return 1; }
    FILE *in = fopen(self, "rb");
#else
    char self[4096];
    ssize_t n = readlink("/proc/self/exe", self, sizeof self - 1);
#if defined(__APPLE__)
    if (n <= 0) { fprintf(stderr, "ASYSMA bootstrap: use macOS app/bootstrap path resolution\n"); return 1; }
#else
    if (n <= 0) { perror("ASYSMA bootstrap: /proc/self/exe"); return 1; }
#endif
    self[n] = '\0';
    FILE *in = fopen(self, "rb");
#endif
    ...
    asysma_native_layout layout;
    long header_pos = 0;
    if (!find_layout(in, &layout, &header_pos)) {
        fprintf(stderr, "ASYSMA bootstrap: no valid ASYSMAEX layout\n"); fclose(in); return 1;
    }
    if (layout.payload_size == 0 || layout.payload_offset < (uint64_t)header_pos ||
        layout.payload_offset + layout.payload_size < layout.payload_offset) {
    ...
    if (!GetTempPathA(sizeof temp, temp) || !GetTempFileNameA(temp, "asy", 0, payload)) {
        fprintf(stderr, "ASYSMA bootstrap: temporary path unavailable\n"); fclose(in); return 1;
    }
    fclose(in);
    in = fopen(self, "rb");
    if (!in || !copy_payload(in, layout.payload_offset, layout.payload_size, payload)) {
        if (in) fclose(in); DeleteFileA(payload); return 1;
    }
    fclose(in);
    SetFileAttributesA(payload, FILE_ATTRIBUTE_NORMAL);
    ...
    DWORD code = 1; GetExitCodeProcess(pi.hProcess, &code); CloseHandle(pi.hProcess); DeleteFileA(payload); return (int)code;
#else
    char payload[] = "/tmp/asysma-native-XXXXXX";
    int fd = mkstemp(payload);
    if (fd < 0) { perror("ASYSMA bootstrap: mkstemp"); fclose(in); return 1; }
    close(fd);
    if (!copy_payload(in, layout.payload_offset, layout.payload_size, payload)) { fclose(in); unlink(payload); return 1; }
    fclose(in);
    if (chmod(payload, 0700) != 0) { perror("ASYSMA bootstrap: chmod"); unlink(payload); return 1; }
#if defined(__APPLE__)
    pid_t pid; char *const child_argv[] = { payload, NULL };
    int rc = posix_spawn(&pid, payload, NULL, NULL, child_argv, environ);
...
#else
    execl(payload, payload, (char *)NULL);
    perror("ASYSMA bootstrap: exec"); unlink(payload); return 1;
#endif
#endif
}
