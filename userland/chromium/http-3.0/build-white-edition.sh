...
# Keeps upstream source separate from generated output and project config.
set -euo pipefail
...
check_source() {
  ...
  [[ -f "$CHROMIUM_SRC/args.gn" ]] && fail "unexpected source-root args.gn; use the output directory instead"
}
...
# Generated output belongs outside the source tree.
...
# Enable only after the patched Chromium source declares these arguments.
