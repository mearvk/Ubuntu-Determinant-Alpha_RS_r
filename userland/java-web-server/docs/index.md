...
<head>
  ...
  <style>
    :root{
      ...
      --line-color: rgba(153,102,255,0.95);
      --line-fill: rgba(153,102,255,0.18);
    }
...
<body>
  ...
  <script>
    (function () {
      ...
      var owner = 'mearvk';
      var repo = 'Java.Web.Server.Telnet.Front.Java.21';
      var commitsCanvas = document.getElementById('commitsChart');
      var ctx = commitsCanvas.getContext('2d');
      var summaryEl = document.getElementById('commitSummary');
      ...
      async function fetchCommitsSince(sinceIso) {
        ...
        while (url && attempts < 200) {
          ...
          var parsed = parseLinkHeader(link);
      ...
      async function updateCommits() {
        try {
          ...
          var totalCommits = (cumulative && cumulative.length) ? (cumulative[cumulative.length - 1] || 0) : 0;
          var updatedAt = new Date();
          summaryEl.textContent = String(totalCommits) + ' commits (cumulative, last 52 weeks) · updated ' + updatedAt.toLocaleString();
          ...
          try {
            ...
            if (repoRes.ok) {
              ...
              var displayName = (repoJson && repoJson.full_name) ? String(repoJson.full_name) : (owner + '/' + repo);
              var stars = (repoJson && typeof repoJson.stargazers_count === 'number') ? repoJson.stargazers_count : 0;
              var forks = (repoJson && typeof repoJson.forks_count === 'number') ? repoJson.forks_count : 0;
              summaryEl.textContent = displayName + ' · ' + stars + ' ★ · ' + forks + ' forks';
              noteEl.textContent = 'Weekly commit data unavailable; showing repo summary.';
            } else {
              summaryEl.textContent = owner + '/' + repo;
              noteEl.textContent = 'Unable to fetch commit data (HTTP ' + repoRes.status + '). Check network or token.';
            }
          } catch (e2) {
            summaryEl.textContent = owner + '/' + repo;
      ...
      var REFRESH_INTERVAL_MS = 2 * 60 * 1000; // 2 minutes
      updateCommits().catch(function(e){ console.warn('Initial update failed', e); });
      setInterval(function(){ updateCommits().catch(function(e){ console.warn('Scheduled update failed', e); }); }, REFRESH_INTERVAL_MS);
