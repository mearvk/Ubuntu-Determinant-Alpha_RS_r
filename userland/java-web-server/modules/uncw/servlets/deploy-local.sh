...
if type nwe_deploy_module &>/dev/null; then
    nwe_deploy_module "UNCW" "uncw" "$SCRIPT_DIR/servlet/src/main/webapp" "" "$TOMCAT_HOME" "$NWE_ROOT"
else
    DEPLOY_DIR="$TOMCAT_HOME/webapps/uncw"; echo "[*] Deploying UNCW™ to $DEPLOY_DIR"
    ...
    chown -R tomcat:tomcat "$DEPLOY_DIR" 2>/dev/null || true; echo "[OK] UNCW deployed at /uncw"
fi
