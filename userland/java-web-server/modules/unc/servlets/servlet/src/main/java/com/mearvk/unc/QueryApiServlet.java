package com.mearvk.unc;

import jakarta.servlet.*; import jakarta.servlet.http.*;
import java.io.*; import java.sql.*; import java.util.Properties;

/**
 * QueryApiServlet — REST endpoint for UNC Chapel Hill query submission.
 * POST /api/query with parameters: school, text
 */
public class QueryApiServlet extends HttpServlet {
    @Override protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        resp.setContentType("application/json;charset=UTF-8");
        String school = req.getParameter("school"), text = req.getParameter("text");
        if (school == null || text == null || text.trim().isEmpty()) { resp.setStatus(400); resp.getWriter().write("{\"error\":\"school and text required\"}"); return; }
        try (Connection conn = getConn(req.getServletContext());
             PreparedStatement ps = conn.prepareStatement("INSERT INTO school_queries (school, query_text) VALUES (?, ?)", Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, school); ps.setString(2, text.trim()); ps.executeUpdate();
