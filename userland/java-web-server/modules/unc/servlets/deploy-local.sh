#!/bin/bash
# UNCChapelHill™ — Deploy Local
# Deploys JSP webapp to Tomcat with JDBC connector and compiled servlet classes.
# Usage: bash modules/unc/servlets/deploy-local.sh [tomcat_home]
set -e
...
echo "[*] Deploying UNCChapelHill™ to $DEPLOY_DIR"
...
rm -rf "$DEPLOY_DIR"
mkdir -p "$DEPLOY_DIR/WEB-INF/lib" "$DEPLOY_DIR/WEB-INF/classes/com/mearvk/unc"
...
JAVA_SRC="$SCRIPT_DIR/servlet/src/main/java/com/mearvk/unc"
...
[ ! -f "$DEPLOY_DIR/WEB-INF/web.xml" ] && echo "[!] WARNING: No WEB-INF/web.xml"
JSP_COUNT=$(find "$DEPLOY_DIR" -name "*.jsp" 2>/dev/null | wc -l)
[ "$JSP_COUNT" -gt 0 ] && echo "[✓] JSP pages: $JSP_COUNT" || echo "[!] WARNING: No JSP files"

echo "[OK] UNCChapelHill™ deployed at /$CONTEXT"
