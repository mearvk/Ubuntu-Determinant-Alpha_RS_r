#!/bin/bash
# Gray Port Registry™ — Module Install Script
...
echo "═══════════════════════════════════════════════════════════════"
echo " Gray Port Registry™ — Module Install"
echo "═══════════════════════════════════════════════════════════════"
echo ""
...
if [ -f "$MOD_ROOT/servlets/setup-db.sh" ]; then
    bash "$MOD_ROOT/servlets/setup-db.sh" && echo "  [✓] Database nwe_gray_registry ready" || echo "  [!] Database setup had issues"
else
...
if type nwe_ensure_ufw &>/dev/null; then
    ...
    sudo ufw allow 9999/tcp >/dev/null 2>&1 && echo "  [✓] Port 9999 (backend) opened" || true
else
    ...
    if command -v ufw &>/dev/null; then
        ...
        sudo ufw allow 9999/tcp >/dev/null 2>&1
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo " Gray Port Registry™ install complete."
echo " Backend port: 9999"
echo " Frontend:     http://localhost:8080/gray-registry/"
echo "═══════════════════════════════════════════════════════════════"
