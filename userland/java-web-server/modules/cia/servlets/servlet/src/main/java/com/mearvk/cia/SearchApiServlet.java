...
public class SearchApiServlet extends HttpServlet {
    @Override protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (Connection conn = getConn(req.getServletContext());
             PreparedStatement ps = conn.prepareStatement("SELECT id, category, LEFT(report_text,200), status, created_at FROM intelligence_reports WHERE report_text LIKE ? OR category LIKE ? ORDER BY created_at DESC LIMIT 50")) {
