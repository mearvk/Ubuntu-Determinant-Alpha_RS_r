#!/bin/bash
# Gray85 Crème Registry™ — Module Install Script
...
echo "═══════════════════════════════════════════════════════════════"
echo " Gray85 Crème Registry™ — Module Install"
echo "═══════════════════════════════════════════════════════════════"
echo ""
...
if [ -f "$MOD_ROOT/servlets/setup-db.sh" ]; then
    bash "$MOD_ROOT/servlets/setup-db.sh" && echo "  [✓] Database nwe_gray85_registry ready" || echo "  [!] Database setup had issues"
else
...
if type nwe_ensure_ufw &>/dev/null; then
    ...
    sudo ufw allow 10085/tcp >/dev/null 2>&1 && echo "  [✓] Port 10085 (backend) opened" || true
else
    ...
    if command -v ufw &>/dev/null; then
        ...
        sudo ufw allow 10085/tcp >/dev/null 2>&1
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo " Gray85 Crème Registry™ install complete."
echo " Backend port: 10085"
echo " Frontend:     http://localhost:8080/gray85-registry/"
echo "═══════════════════════════════════════════════════════════════"
