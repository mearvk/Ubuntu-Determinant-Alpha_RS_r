#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════════
# NitroWebExpress™ — SpectrumTandem Backend Startup
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════════════════╗"
echo "║  SpectrumTandem Backend Server — Startup                                ║"
echo "║  JVM: $JVM_OPTS                                                            ║"
echo "╚═══════════════════════════════════════════════════════════════════════════╝"
echo ""
...
# ── Module definition ─────────────────────────────────────────────────────────
MODULE_CLASS="source.SpectrumTandemServer"
...
while [ $SECONDS -lt $DEADLINE ]; do
    if timeout 1 bash -c "echo >/dev/tcp/localhost/49222" 2>/dev/null; then
        READY=1; break
    fi
    sleep 1
done
...
if [ $READY -eq 1 ]; then
    echo "✓ (PID $PID, port 49222 UP)"
elif kill -0 "$PID" 2>/dev/null; then
    echo "~ (PID $PID alive, port 49222 not yet bound — timeout)"
else
    echo "✗ (FAILED)"
    rm -f "$PID_FILE"
    echo ""
