#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════════
# NitroWebExpress™ — Dictionary™ Deploy Local
...
DEPLOY_DIR="$TOMCAT_HOME/webapps/dictionary"

echo "[*] Deploying Dictionary™ to $DEPLOY_DIR"
rm -rf "$DEPLOY_DIR"
...
[ -z "$JDBC_JAR" ] && JDBC_JAR=$(find "$NWE_ROOT" -path "*/WEB-INF/lib/mysql-connector-j*.jar" -type f 2>/dev/null | head -1)
[ -n "$JDBC_JAR" ] && cp "$JDBC_JAR" "$DEPLOY_DIR/WEB-INF/lib/"

chown -R tomcat:tomcat "$DEPLOY_DIR" 2>/dev/null || true
echo "[OK] Dictionary™ deployed at /dictionary"
