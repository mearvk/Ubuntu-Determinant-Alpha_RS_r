#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════════
# NitroWebExpress™ — Vietnam Deploy Local
# Deploys JSP webapp to Tomcat with JDBC connector.
# Usage: bash modules/vietnam/servlets/deploy-local.sh [tomcat_home]
# ═══════════════════════════════════════════════════════════════════════════════════
set -e
...
if type nwe_deploy_module &>/dev/null; then
    nwe_deploy_module "Vietnam" "vietnam" \
        "$SCRIPT_DIR/servlet/src/main/webapp" \
        "" \
        "$TOMCAT_HOME" "$NWE_ROOT"
else
    # Fallback if shared library unavailable
    DEPLOY_DIR="$TOMCAT_HOME/webapps/vietnam"
    echo "[*] Deploying Vietnam™ to $DEPLOY_DIR"
    rm -rf "$DEPLOY_DIR"
    ...
    echo "[OK] Vietnam deployed at /vietnam"
fi
