#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════════
# NitroWebExpress™ — Vietnam Backend Startup
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════════════════╗"
echo "║  Vietnam Backend Server — Startup                                       ║"
echo "║  JVM: $JVM_OPTS                                                            ║"
echo "╚═══════════════════════════════════════════════════════════════════════════╝"
echo ""
...
# ── Module definition ─────────────────────────────────────────────────────────
MODULE_CLASS="source.VietnamServer"
...
while [ $SECONDS -lt $DEADLINE ]; do
    if timeout 1 bash -c "echo >/dev/tcp/localhost/49215" 2>/dev/null; then
        READY=1; break
    fi
    sleep 1
done
...
if [ $READY -eq 1 ]; then
    echo "✓ (PID $PID, port 49215 UP)"
elif kill -0 "$PID" 2>/dev/null; then
    echo "~ (PID $PID alive, port 49215 not yet bound — timeout)"
else
    echo "✗ (FAILED)"
    rm -f "$PID_FILE"
    echo ""
