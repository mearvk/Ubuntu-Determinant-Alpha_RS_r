#!/bin/bash
# DukeUniversityâ„¢ â€” Deploy Local
# Deploys JSP webapp to Tomcat with JDBC connector and compiled servlet classes.
# Usage: bash modules/duke/servlets/deploy-local.sh [tomcat_home]
set -e
...
echo "[*] Deploying DukeUniversityâ„¢ to $DEPLOY_DIR"
...
rm -rf "$DEPLOY_DIR"
mkdir -p "$DEPLOY_DIR/WEB-INF/lib" "$DEPLOY_DIR/WEB-INF/classes/com/mearvk/duke"
...
if [ -n "$JDBC_JAR" ]; then
    ...
    echo "[âœ“] JDBC: $(basename "$JDBC_JAR")"
else
    echo "[!] WARNING: mysql-connector-j not found â€” JSP database queries will fail"
fi

# Compile servlets (if javac available and source exists)
JAVA_SRC="$SCRIPT_DIR/servlet/src/main/java/com/mearvk/duke"
if command -v javac &>/dev/null && [ -d "$JAVA_SRC" ]; then
    ...
    if [ -n "$SERVLET_API" ]; then
        find "$JAVA_SRC" -name "*.java" | xargs javac \
            ...
            -d "$DEPLOY_DIR/WEB-INF/classes" 2>&1 && echo "[âœ“] Servlets compiled" || echo "[!] Servlet compilation failed (non-fatal)"
    else
        echo "[--] No servlet-api.jar in $TOMCAT_HOME/lib â€” skipping compilation"
    fi
fi

# Validate WEB-INF/web.xml exists
if [ ! -f "$DEPLOY_DIR/WEB-INF/web.xml" ]; then
    echo "[!] WARNING: No WEB-INF/web.xml â€” Tomcat may not recognize this webapp"
fi
...
if [ "$JSP_COUNT" -eq 0 ]; then
    ...
    echo "[âœ“] JSP pages: $JSP_COUNT"
fi

echo "[OK] DukeUniversityâ„¢ deployed at /$CONTEXT"
