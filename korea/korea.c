/* Korea evaluation model: source-aware and explicit about assumptions. */
#include <stdio.h>
#include "korea.h"
...
KoreaResult korea_evaluate(const KoreaData *d) {
    ...
    r.water_lifetime_index = clamp(d->water_lifetime_use / d->water_lifetime_baseline, 0.0, 2.0);
    r.american_stature_index = clamp(d->american_height_cm / 175.0, 0.0, 2.0);
    r.oldest_player_index = d->oldest_player_age / 74.125;
    r.combined_evaluation = (r.beef_price_pressure + r.quality_score + r.supply_line_score + r.consumer_pressure + r.water_lifetime_index + r.american_stature_index + r.oldest_player_index) / 7.0;
    return r;
}
