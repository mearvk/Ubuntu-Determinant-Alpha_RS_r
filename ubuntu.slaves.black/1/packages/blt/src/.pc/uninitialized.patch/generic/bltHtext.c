...
static Tk_ClassProcs htextClass = {
			         ...
			         * already have values for some fields. */
{
    ...
    if (Blt_ConfigModified(configSpecs, interp, "-font", "-linespacing", "-file",
     ...
     */
    if (Blt_ConfigModified(configSpecs, interp, "-file", "-text", (char *)NULL)) {
	int result;
...
{
    HText *htPtr = (HText *)clientData;
    htPtr->flags |= (TEXT_DIRTY | REQUEST_LAYOUT);
    ConfigureText(htPtr->interp, htPtr);
    EventuallyRedraw(htPtr);
}

/* Layout Procedures */
    ...
    char **argv;		/* Argument strings. */
{
    ...
    Tk_SetClassProcs(tkwin, &htextClass, (ClientData)htPtr);
    return TCL_OK;
}
