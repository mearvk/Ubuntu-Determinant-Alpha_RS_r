...
typedef struct {
    ...
    int flags;			/* Various flags;  see below for
				 * definitions. */
    Blt_Tile tile, tile2, activeTile;
    char *imagePtr;
    Tk_Image image;
} Scrollbar;
...
static Tk_ConfigSpec configSpecs[] =
{
	...
	&bltTileOption},
    {TK_CONFIG_CUSTOM, "-tile2", "tile2", "Tile2",
	(char *)NULL, Tk_Offset(Scrollbar, tile2), TK_CONFIG_NULL_OK,
	&bltTileOption},
...
static Tk_ClassProcs scrollClass = {
...
widgetWorldChanged(ClientData clientData)
{
    ...
    scrollPtr->flags |= (SCROLLBAR_LAYOUT);
    ...
    int stype;
{
    ...
    scrollPtr = Blt_Calloc(1, sizeof(Scrollbar));
    ...
    Tk_SetClassProcs(tkwin, &scrollClass, (ClientData)scrollPtr);
    ...
    DestroyData *memPtr;	/* Info about scrollbar widget. */
{
    ...
    if (scrollPtr->tile2 != NULL) {
        Blt_FreeTile(scrollPtr->tile2);
    }
    if (scrollPtr->image != NULL) {
        Tk_FreeImage(scrollPtr->image);
    }
					 ...
					 * Tk_ConfigureWidget. */
{
    ...
    */
    ...
    if (scrollPtr->imagePtr != NULL) {
        image = Tk_GetImage(scrollPtr->interp, scrollPtr->tkwin,
            scrollPtr->imagePtr, ScrollbarImageProc,
    ...
    if (scrollPtr->tile2 != NULL) {
        Blt_SetTileChangedProc(scrollPtr->tile2, TileChangedProc,
            (ClientData)scrollPtr);
    }
    ...
    ClientData clientData;	/* Information about window. */
{
    ...
    Pixmap pixmap;
    Blt_Tile tile, tiletr, tilecur;

    tiletr = (Blt_HasTile(scrollPtr->tile) ? scrollPtr->tile : NULL);
    tile = (Blt_HasTile(scrollPtr->tile2) ? scrollPtr->tile2 : tiletr);
    ...
    if (tiletr != NULL) {
	Blt_SetTileOrigin(tkwin, tiletr, 0, 0);
	Blt_TileRectangle(tkwin, pixmap, tiletr, scrollPtr->inset, 
		scrollPtr->inset,
	...
	relief = scrollPtr->activeField == TOP_ARROW ? scrollPtr->activeRelief
	    : TK_RELIEF_RAISED;
	if (Blt_HasTile(scrollPtr->activeTile)) {
	    Blt_SetTileOrigin(tkwin, scrollPtr->activeTile, 0, 0);
	    tilecur = scrollPtr->activeTile;
	}
    } else {
    ...
    if (tilecur != NULL) {
	Blt_TilePolygon(tkwin, pixmap, tilecur, points, 3);
...
#else
    Blt_Fill3DRectangleTile(tkwin, pixmap, border, scrollPtr->inset, 
       scrollPtr->inset, width, width, elementBorderWidth, relief, tilecur, 0, 0); 
...
#else
    Blt_Fill3DRectangleTile(tkwin, pixmap, border, 
            Tk_Width(tkwin) - (width + scrollPtr->inset), 
            Tk_Height(tkwin) - (width + scrollPtr->inset),
            width, width, elementBorderWidth, relief, tilecur, 0, 0); 
    ...
    if (scrollPtr->vertical) {
	if (tilecur) {
	    Blt_TileRectangle(tkwin, pixmap, tilecur, scrollPtr->inset, 
	...
	if (tilecur) {
	    Blt_TileRectangle(tkwin, pixmap, tilecur, scrollPtr->sliderFirst, 
		scrollPtr->inset,
    ...
    if (scrollPtr->image != NULL) {
        /* Draw adornment image centered on bar. */
        int imgW, imgH, xLen, yLen, yStart, xStart, yOfs = 0, xOfs = 0, pad = 1 + scrollPtr->borderWidth;
        
        Tk_SizeOfImage(scrollPtr->image, &imgW, &imgH);
        
        if (scrollPtr->vertical) {
            xLen = width;
            yLen = scrollPtr->sliderLast - scrollPtr->sliderFirst;
        } else {
            yLen = width;
            xLen = scrollPtr->sliderLast - scrollPtr->sliderFirst;
        ...
        xStart = (xLen - imgW)/2+pad;
        yStart = (yLen - imgH)/2+pad;
        if (scrollPtr->vertical) {
            yStart += scrollPtr->sliderFirst;
        } else {
            xStart += scrollPtr->sliderFirst;
        }
        if (xStart>=0 && yStart>=0 && imgW > 0 && imgH > 0) {
            Tk_RedrawImage(scrollPtr->image, xOfs, yOfs, imgW, imgH, pixmap, xStart, yStart);
        }
    }
