    ...
    int offset;			/* Offset to field in structure */
{
    ...
    tkAdopted = NULL;
    window = None;
    if ((flags & SEARCH_TKWIN) && string && ((string[0] == '.') ||
        ((!strncmp(string, "interp", 6)) && ((cp = strchr(string, '.')))))) {
	Tk_Window tkwin;
	Tcl_Interp *slave = NULL;

        if (string[0] != '.') {
            /* Accept an interp prefix before the window. */
            strncpy(buf, string, (cp-string));
            buf[(cp-string)] = 0;
            string = cp;
            slave = Tcl_GetSlave(interp, buf);
            if (!slave) {
                Tcl_AppendResult(interp, "cant find slave: ", buf,
                    (char *)NULL);
                    return TCL_ERROR;
            }
        } else {
        ...
        tkwin = Tk_NameToWindow(slave, string, Tk_MainWindow(slave));
        if (tkwin == NULL) {
	    return TCL_ERROR;
	}
	if (!Tk_IsTopLevel(tkwin)) {
	...
	if (!Tk_IsMapped(tkwin)) {
	    /* Forced mapping fails to get dimensions. */
	    /* Tk_MapWindow(tkwin); */
            Tcl_AppendResult(interp, "Tk window unmapped, try tkwait visibility ",
                            string, (char *)NULL);
                 return TCL_ERROR;
	}
    ...
    int flags;
{
    ...
    if (Blt_ConfigModified(configSpecs, interp, "-window", "-name", "-command", 
			   (char *)NULL)) {
	cntrPtr->flags &= ~CONTAINER_MAPPED;
	if (cntrPtr->adopted != None) {
