...
#define PAD	8
     ...
     */
     ...
     if ((EpsCoords(interp, canvas, itemPtr, i, objv) != TCL_OK)) {
        return TCL_ERROR;
    }

    if (ConfigureEps(interp, canvas, itemPtr, argc - i, objv + i, 0)
	!= TCL_OK) {
	DeleteEps(canvas, itemPtr, Tk_Display(tkwin));
	return TCL_ERROR;
    }
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
     ...
     */
    if (Blt_ConfigModified(configSpecs, interp, "-image", (char *)NULL)) {
	if (epsPtr->preview != NULL) {
    ...
    if (Blt_ConfigModified(configSpecs, interp, "-file", (char *)NULL)) {
	CloseEpsFile(epsPtr);
	if (epsPtr->pixmap != None) {
#ifdef notyet
	    Blt_FreeColorTable(epsPtr->colorTable);
    ...
    if (Blt_ConfigModified(configSpecs, interp, "-quick", (char *)NULL)) {
	epsPtr->lastWidth = epsPtr->lastHeight = 0;
    }
    /* Fill color GC */
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc > 2) {
	Tcl_AppendResult(interp, "wrong # coordinates: expected 0, 1, or 2, got ",
	    Blt_Itoa(argc), (char *)NULL);
	return TCL_ERROR;
    }
    if (argc==1) {
        if (Tcl_ListObjGetElements(interp, objv[0], &argc,
            (Tcl_Obj ***) &objv) != TCL_OK) {
                return TCL_ERROR;
        } else if (argc != 2) {
            char buf[64];
