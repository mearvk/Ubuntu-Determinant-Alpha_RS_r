...
Depends: dpkg dependency field syntax
    ...
    As a special case, if ``build-essential`` appears as a dependency
    and an architecture other than the testbed's native architecture
    is to be tested (see `Cross-testing` below), it will be replaced with
    ...
    avoided by specifying ``build-essential:native``.
...
Some packages and architectures are most frequently used via a foreign
architecture, for example i386 packages installed on an amd64 system.
To test these packages in a realistic way, their tests can be run in a
testbed that is natively for one architecture (typically amd64), with the
different architecture under test (for example i386) added as a foreign
architecture.

Tests can detect whether they are being run like this by checking whether
one of the ``DEB_HOST_*`` family of environment variables from
``dpkg-architecture`` is set. If so:

- All the ``DEB_HOST_*`` variables from ``dpkg-architecture`` are set
  according to the architecture under test. However, the ``DEB_BUILD_*``
  and ``DEB_TARGET_*`` variables are not set.

- The test may assume that executables that were built for the
  architecture under test can be run transparently, for example via
  kernel support for alternative ABIs (such as i386 in an amd64 testbed)
  or via transparent emulation mechanisms like ``qemu-user-binfmt`` or
  ``qemu-user-static`` (such as arm64 in an amd64 testbed).
...
- Binary packages from the source package under test are always installed
  for the test architecture, as though they had been specified as
  ``foo:${DEB_HOST_ARCH}``.

- Other dependencies in the ``Depends`` field are treated like
  cross build-dependencies, with the testbed architecture taking the place
  of the build architecture and the architecture under test taking the
  place of the host architecture:

    - ``Multi-Arch: foreign`` packages are installed for the testbed
      architecture
    - All other packages are installed for the architecture under test,
      unless qualified with ``:native`` in which case the testbed
      architecture is used

- If the test is intended to invoke a compiler to compile code against
  the package under test, it must use a ``${DEB_HOST_GNU_TYPE}``
  compiler (which might be a cross-compiler), for example
  ``${DEB_HOST_GNU_TYPE}-gcc``. Similarly, if it invokes pkg-config, it
  must use ``${DEB_HOST_GNU_TYPE}-pkg-config`` or set the
  ``PKG_CONFIG_LIBDIR`` or ``PKG_CONFIG_PATH``.

- If the test uses ``apt`` to install or manipulate the package under test,
  it must use ``foo:${DEB_HOST_ARCH}`` to install the correct architecture.
