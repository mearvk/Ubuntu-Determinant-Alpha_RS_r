...
class Testbed:
    def __init__(self, vserver_argv, output_dir, user,
                 ...
                 enable_apt_fallback=True, shell_fail=False, needs_internet='run', cross_arch=None):
        self.sp = None
        self.lastsend = None
        self.scratch = None
        self.modified = False
        ...
        self._guessed_default_release = None
    ...
    def _opened(self, pl):
        ...
        # set up environment for cross-architecture if needed
        if self.cross_arch:
            self.cross_env = []
            argv = ['dpkg-architecture', '-a', self.cross_arch]
            # ignore stderr
            (code, vars, err) = self.execute(argv,
                                             stdout=subprocess.PIPE,
                                             stderr=subprocess.PIPE)
            if code != 0:
            ...
            for var in vars.split('\n'):
                if var.startswith('DEB_HOST'):
                    self.cross_env.append(var)
            adtlog.info('testbed target architecture: ' + self.cross_arch)
    ...
    def install_deps(self, deps_new, shell_on_failure=False, synth_deps=[]):
        ...
        if self.cross_arch:
            # mock up a .dsc with our dependencies string as our build-deps
            dsc = TempPath(self, 'autopkgtest-satdep.dsc')
            with open(dsc.host, 'w', encoding='UTF-8') as f_dsc:
                f_dsc.write("Source: autopkgtest-satdep\n"
                            "Binary: autopkgtest-satdep\n"
                            "Architecture: any all\n"
                            "Build-Depends: " + ', '.join(deps_new) + "\n")
            dsc.copydown()

            # feed the result to apt-get build-dep
            self.satisfy_build_deps(dsc.tb, shell_on_failure)
            # Now handle the synthetic deps as well
            self.install_apt('', shell_on_failure, synth_deps)
        else:
...
''' % (self.dpkg_arch, deps))
    ...
    def _run_apt_build_dep(self, what, prefix, recommends, ignorerc=False):
        '''actually run apt-get build-dep'''

        # capture status-fd to stderr
        (rc, _, serr) = self.execute(['/bin/sh', '-ec', '%s apt-get build-dep '
                                      '--assume-yes %s '
                                      '-o APT::Status-Fd=3 '
                                     ...
                                     kind='install', stderr=subprocess.PIPE)
        if not ignorerc and rc != 0:
            adtlog.debug('apt-get build-dep %s failed; status-fd:\n%s' % (what, serr))
            # check if apt failed during package download, which might be a
            # transient error, so retry
            if 'dlstatus:' in serr and 'pmstatus:' not in serr:
                raise adtlog.AptDownloadError
    ...
    def satisfy_build_deps(self, source_pkg, recommends=False,
                           shell_on_failure=False):
        '''Install build-dependencies into testbed with apt-get build-dep
        ...
        if not can_apt_get:
            self.bomb('no root on testbed, unsupported when specifying target architecture')

        what = source_pkg
        if self.cross_arch:
            what += ' -a ' + self.cross_arch
        # install the build-dependencies for the specified source (which can
        # be a source package name, or a path); our apt pinning is not
        # very clever wrt. resolving transitional dependencies in the pocket,
        # so we might need to retry without pinning
        download_fail_retries = 3
        while True:
            rc = 0
            try:
                self._run_apt_build_dep(what, ' '.join(self.eatmydata_prefix), recommends)
            ...
            if rc != 0:
                if self.apt_pin_for_releases and self.enable_apt_fallback:
                    release = self.apt_pin_for_releases.pop()
                    adtlog.warning('Test dependencies are unsatisfiable using apt pinning. '
                                   'Retrying using all packages from %s' % release)
                    self.check_exec(['/bin/sh', '-ec', 'rm /etc/apt/preferences.d/autopkgtest-' + release])
                    if not self.apt_pin_for_releases:
                        self.check_exec(['/bin/sh', '-ec', 'rm -f /etc/apt/preferences.d/autopkgtest-default-release'])
                    continue
                ...
                adtlog.warning('Test dependencies are unsatisfiable - calling '
                               'apt install on test deps directly for further '
                               'data about failing dependencies in test logs')
                self._run_apt_build_dep('--simulate ' + what,
                                        ' '.join(self.eatmydata_prefix),
    ...
    def _create_apt_pinning_for_packages(self, release, pkglist, default_releases=[]):
        ...
        if srcpkgs:
            ...
            if self.cross_arch:
                script += 'PKGS="$PKGS $(apt-cache showsrc %s | ' \
                    '''awk '/^Package-List:/ { show=1; next } (/^ / && show==1) { print $1 ":%s"; next } { show=0 }' |''' \
                    '''sort -u | tr '\\n' ' ')"; ''' % \
                    (' '.join(srcpkgs), self.cross_arch)
    ...
    def _get_default_release(self):
        ...
        # The default release has been specified via --apt-default-release.
        if self.default_release:
            return self.default_release

        # Return the cached value of the guessed default release.
        if self._guessed_default_release:
            return self._guessed_default_release

        # Guess the default release by parsing the sources.list files.
        # The first encountered release will be considered the default release.
        ...
        self._guessed_default_release = self.check_exec(['sh', '-ec', script], stdout=True).strip()
