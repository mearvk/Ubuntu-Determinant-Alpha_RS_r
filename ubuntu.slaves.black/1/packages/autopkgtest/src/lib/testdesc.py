...
def _debian_packages_from_source(srcdir, cross_arch=None):
    packages = []
    packages_no_arch = []
    ...
    for st in parse_rfc822(os.path.join(srcdir, 'debian/control')):
        ...
        qual_pkg = st['Package']
        # take care to emit an arch qualifier only for arch-dependent
        # packages, not for arch: all ones
        if cross_arch:
            qual_pkg += ':' + cross_arch
        if arch == 'all':
            packages.append(st['Package'])
        elif arch == 'any':
            packages.append(qual_pkg)
        else:
            packages.append('%s [%s]' % (qual_pkg, arch))
    ...
    for alt_group_str in dep_str.split(','):
        ...
        if alt_group_str == '@':
            for d in my_packages:
                ...
                deps.append(d)
                s = _synthesize_deps(d, target_arch)
                if s:
                    synthdeps.append(s)
            ...
            for d in _debian_recommends_from_source(srcdir, testbed_arch):
                adtlog.debug('synthesised dependency %s' % d)
                deps.append(d)
        elif alt_group_str == 'build-essential':
            # special case; this is how packages declare they want to build
            # code during the test (but not necessarily doing a full package
            # build), but for cross-architecture testing this declaration is
            # wrong because it tries to pull in build-essential for the wrong
            # arch.  So we apply the same fix-up here that we do in
            # runner/autopkgtest.  We can't expect the package maintainer to
            # do this because they don't know what crossbuild-essential-$arch
            # to pull in.
            deps.append('build-essential:native')
            if cross_arch:
                deps.append('crossbuild-essential-%s:native' % cross_arch)
        else:
            ...
            if cross_arch:
                for mine in my_packages:
                    # ignore [arch] filters for matching packages
                    if alt_group_str + ':' + cross_arch == mine.split(' ')[0]:
                        adtlog.debug('%s is from our source package, adding arch qualifier for cross-testing' % s)
                        alt_group_str += ':' + cross_arch
                        break
        ...
        try:
            ...
            if 'Tests' in record:
                ...
                (depends, synth_depends) = _parse_debian_depends(
                    ...
                    record.get('Depends', '@'),
                    srcdir,
                    testbed_arch, cross_arch=cross_arch)
                ...
                for n in test_names:
                    try:
                        ...
                        _check_architecture(n, target_arch, architectures)
                ...
                _check_architecture(name, target_arch, architectures)
