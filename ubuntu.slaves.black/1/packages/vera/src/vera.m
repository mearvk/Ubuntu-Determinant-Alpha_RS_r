...
@table @asis
@item M2C
Model To Code (M2M, MDSD)

@item M2M
Machine To Machine [protocol]

@item M2M
Model To Model (M2C, MDSD)
...
@item MAMP
Mac, Apache, MySQL, PHP (Mac, Apache, PHP, DB, SQL)
...
@item MAPEC
Multimedia APplications in Education Conference, "MApEC"
...
@item MAPP
Mac, Apache, PostgreSQL, PHP (Mac, Apache, PHP, DB, SQL)

@item MAPP
Microsoft Active Protections Program (MS)

@item MAPS
Microsoft Active Protection System (MS, MSE)

@item MAPS
Multiple Airflow Pass-through Structure (Scythe), M.A.P.S.
...
@item MBA
Mobile Bitstream Access (mobile-systems)
...
@item MBM
Media Bandwidth Management (DSL, Zyxel)
...
@item MBSA
Microsoft Business and Services Agreement (MS)

@item MBUS
Module BUS [standard] (Sun, SPARC), "MBus"

@item MC2UC
MultiCast 2 UniCast (WLAN)
...
@item MCC
Mobile Country Code (mobile-systems)

@item MCC
Monitor and Control Center (Bundeswehr)
...
@item MCE
Machine Check Exception (Intel, CPU)
...
@item MCT
Media Creation Tool (MS, Windows)
...
@item MDA
Mobile Digital Assistant (T-Mobile, PDA)

@item MDA
Model Driven Architecture (CASE, OMG)

@item MDA
Monochrome Display Adapter [720x350] (IBM, PC)
...
@item MDM
Mobile Device Management (CA, Unicenter, MS)

@item MDN
Mozilla Developer Network

@item MDNS
Multicast - Domain Name System (DNS), "mDNS"

@item MDOP
Microsoft Desktop Optimization Pack (MS, Windows)
...
@item MDS
Microsoft Deployment Solution (MS, Windows, Vista)
...
@item MDT
Microsoft Deployment Toolkit (MS)
...
@item ME
Management Engine (Intel, AMT, BIOS, UEFI)
...
@item MEC
Microsoft Exchange Conference (MS)

@item MEF
Managed Extensibility Framework (MS)
...
@item MEWDE
My EMACS Web Development Environment (EMACS, WWW)
...
@item MFC
Memory Flow Controller (SPE, Cell, IBM)
...
@item MIDP
Mobile Information Device Profile (J2ME, Java, JSR-118)
...
@item MIG
Mach Interface Generator (Mach, IDL)
...
@item MINC
Multilingual Internet Names Consortium (org., Internet)
...
@item MLC
Multi Level Cell (IC, PCMCIA, Flash, Intel, SLC)
...
@item MMAPI
Mobile Media API (Java, Sun, JMF, API, JSR-135)
...
@item MMOS
Massive Multiplayer Online Science (MMO)
...
@item MMTC
Massive Machine-Type Communication (MTC, 3GPP), "M-MTC"
...
@item MNC
Mobiles Network Codes (mobile-systems)
...
@item MOBA
Multiplayer Online Battle Arena (ARTS, RTS)

@item MOBI
Mobility Open Blockchain Initiative (org.)
...
@item MODI
Microsoft Office Document Imaging (MS)
...
@item MOLO
Mobile Other Licensed Operator (mobile-systems, MVNO)
...
@item MOOC
Massive Open Online Course (Internet)
...
@item MOPRIA
MObile PRinting Alliance (org., Canon, HP, Samsung, Xerox), "Mopria"
...
@item MOSA
Microsoft Online Subscription Agreement (MS)

@item MOSA
Multiplayer Online Survival Arena
...
@item MOSIX
Multicomputer Operating System for UnIX (Unix, Cluster, MOSIX)
...
@item MPSA
Microsoft Products and Services Agreement (MS)
...
@item MQA
Master Quality Authenticated (audio)

@item MQI
Message Queueing Interface (IBM)

@item MQT
Materialized Query Table (IBM, DB)
...
@item MRC
MOSIX Research the Clouds (MOSIX)
...
@item MSATA
Mini Serial Advanced Technology Attachment (SATA), "mSATA"
...
@item MSC
Microprocessor Standardization Committee (Oerg., IEEE)
...
@item MSE
Media Source Extensions (W3C, HTML, Javascript)

@item MSE
MicroSoft Exchange server (MS)

@item MSE
Microsoft Security Essentials (MS)
...
@item MSL
Master Security Lock (mobile-systems)
...
@item MSPL
MicroSoft Public License (MS, Ms-RL), "Ms-PL"
...
@item MSRC
Microsoft Security Response Center (MS)

@item MSRL
MicroSoft Reciprocal License (MS, Ms-PL), "Ms-RL"
...
@item MSTP
Multiple Spanning Tree Protocol (STP, VLAN, IEEE 802.1s)

@item MSTSC
MicroSoft Terminal Services Client (MS)

@item MSU
Millions of Service Unit (IBM, TC)
...
@item MSXML
Microsoft XML core services (MS, COM, XML,)
...
@item MTC
Machine-Type Communication (3GPP)
...
@item MTP
Message Transfer Part (ISDN, GSM, mobile-systems, SS7)
...
@item MUMIMO
Multi User Multiple Input Multiple Output (WLAN), "MU-MIMO"
...
@item MUTOS
MultiUser-/multitasking Timesharing Operating System (GDR, OS)
...
@item MVNO
Mobile Virtual Network Operator (mobile-systems, MOLO)
...
@item MXM
Mobile PCI eXpress Module (Nvidia, PCI)

@item MXML
Macromedia Flex Markup Language (Macromedia, XML)
