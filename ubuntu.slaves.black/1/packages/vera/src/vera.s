...
@item SACD
Super Audio Compact Disk (CD, audio)
...
@item SAM
Security Account Manager (Samba)
...
@item SAR
Specific Absorption Rate (mobile-systems)
...
@item SASPF
Standard-Anwendungs-Software-Produkt-Familien (Bundeswehr)
...
@item SATAHAT
SATA Hardware Attached on Top (SATA), "SATA-HAT"
...
@item SBF
Single Binary File (Motorola, mobile-systems)
...
@item SCCP
Signaling Connection Control Part (MSC, GSM, mobile-systems, SS7)
...
@item SCE
Sony Computer Entertainment (manufacturer, Sony)
...
@item SCOM
System Center Operations Manager (MS)

@item SCOMO
Software Component Management Object (OMA)
...
@item SCVMM
System Center Virtual Machine Manager (MS)
...
@item SDK
StandardDatenschutzKlausel (DSGVO)
...
@item SDL
Software Development Laboratories (manufacturer, veraltet, Oracle)
...
@item SDLT
Super Digital Line Tape (DLT, Streamer)
...
@item SFC
Software Freedom Conservancy (org.)
...
@item SIGDA
Special Interest Group on Design Automation (org., ACM)
...
@item SIMSA
	Swiss Interactive Media and Software Association (org.)
...
@item SISA
Sony Interactive Studios America (Sony)
...
@item SIUF
System Initiated User Feedback (MS, Windows)
...
@item SL4A
Scripting Layer For Android (Android)
...
@item SLP
Service Location Protocol (IP, RFC 3224)

@item SLP
Service Logic Program (IN)

@item SLP
Substrate Like PCB (PCB)
...
@item SMING
Structure of Management Information - Next Generation (RFC 3780), "SMIng"
...
@item SNTP
Simple Network Time Protocol (NTP, RFC 2030)
...
@item SOOPPA
Symposion on Object-Oriented Programming emPhasizing Practical Applications

@item SOP
Standard Operating Procedure

@item SOPA
Stop Online Piracy Act (Internet)
...
@item SP
Scale-independent Pixel (SIP, DIP, DP)
...
@item SPA
Single Page Application (Android)
...
@item SRTP
Secure Real-time Transport Protocol (RFC 3711, RTP)

@item SRTS
Synchronous Residual Time Stamp

@item SRVCC
Single Radio Voice Call Continuity (LTE, WCDMA)
...
@item SS7
Signaling System 7 (ISDN, 3G)

@item SS7
Signalling System #7 (PSTN, SS7)
...
@item SSM
Source Specific Multicast (IETF, WG, Multicast, RFC 3569)
...
@item SSU
Service Stack Update (MS, Windows)
...
@item STMRAM
Spin Torque Magnetoresistive Random Access Memory (IC, RAM), "ST-MRAM"
...
@item STP
Spanning Tree Protocol (IEEE 802.1D, MSTP)
...
@item SUHD
Samsung Ultra High-Definition (UHD)
...
@item SVGA
Super Video Graphics Array [800x600] (VGA)
...
@item SXRD
Silicon X-tal Reflective Display (LCoS)
