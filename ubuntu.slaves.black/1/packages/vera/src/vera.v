...
@item VAC
Valve Anti Cheat [module/system]

@item VAC
Visual Age C++ (IBM)

@item VACP
View-based Access Control Protocol (SNMP, RFC 2575)
...
@item VAXELN
Virtual Address eXtension Executive for Local area Networks (OS, DEC)
...
@item VBCCE
Visual Basic Control Creation Edition (VB, MS)
...
@item VBO
Vertex Buffer Object (NVSG)

@item VBR
Variable Bit Rate (ATM, CBR, ABR, UBR, QOS, BIT)

@item VBRNRT
Variable Bit Rate - Non Real Time (VBR, ATM, BIT), "VBR-NRT"

@item VBRRT
Variable Bit Rate - Real Time (VBR, ATM, BIT), "VBR-RT"
...
@item VCE
Vision Computing Engine

@item VCEG
Video Coding Experts Group (org., ITU-T)
...
@item VCVS
Virtual Concurrent Version System (CVS), "vCVS"
...
@item VDDM
Vista Display Driver Model (MS, Windows, Vista)

@item VDE
Verband Deutscher Elektrotechniker [e.v.] (org.)

@item VDGA
Virtual Dedicated Graphics Acceleration (VMware, vSGA), "vDGA"
...
@item VDI
Virtual Desktop Infrastructure (Sun)
...
@item VDOP
Vertical Dilution Of Precision (GPS)

@item VDPAU
Video Decode and Presentation API for Unix (Nvidia, API)

@item VDS
Virtual DMA Services (DMA)

@item VDSL
Very high data / bit rate Digital Subscriber Line (DSL, BIT)
...
@item VDV
Vorbestellte DauerwaehlVerbindung (DTAG)

@item VE
Virtual Environment (VR)

@item VEBSN
Virtual European Seismic Network
...
@item VEP
Visual Editor Project (GUI, Java, Eclipse)
...
@item VFAT
Virtual File Allocation Table (MS, Windows 95, FAT)
...
@item VFIO
Virtual Function Input-Output (Linux, IO)
...
@item VFS
Virtual Filesystem Switch (FreeBSD)
...
@item VGPU
Virtual Graphics Processing Unit (GPU), "vGPU"
...
@item VIC
Video Interface Controller (video, C64)
...
@item VID
Voltage IDentification (CPU)

@item VIDIX
VIDeo interface for *niX (Unix, video)
...
@item VIOLA
Vertically Intergrated Optical Testbed for Large Applications (DFN)

@item VIOP
Variable Input/Output Port (I/O, Qualstar)

@item VIP
VINES Internetwork Protocol (VINES)

@item VIP
Virtual Internet Protocol [address] (IP)
...
@item VIPT
Virtually Indexed, Physically Tagged (Intel, CPU)
...
@item VKI
Variable Keyframe Interface (DivX, video)

@item VKI
Verteilte Kuenstliche Intelligenz (KI, DAI)

@item VL
Virtual Lane (Infiniband)

@item VLA
Volume Licence Agreement (Novell, Apple)

@item VLAN
Virtual Local Area Network (LAN, IEEE 802.1q/80210), vowlan

@item VLB
VESA Local Bus (VESA)

@item VLC
Variable Length Coding (AVC)

@item VLC
Video LAN Client (LAN, video, Mac, Dbox)
...
@item VMA
VSphere Management Assistant (VSAN), "vMA"
...
@item VMC
Virtual Machine Console (VMware)
...
@item VME
Virtual Mobile Engine (Sony, PSP)

@item VMESA
Virtual Machine / Enterprise Systems Architecture (IBM, VM, ESA, CMS), "VM/ESA"

@item VML
Vector Markup Language (XML, MS, Macromedia, ..., PGML, SVG)
...
@item VNNI
Vector Neural Network Instructions (AVX, CPU)
...
@item VOICE
Virtual OS/2 International Consumer Education (OS/2, user group, org.)
...
@item VOWIP
Voice Over Wireless Internet Protocol (LAN, IP), "VoWIP"

@item VOWLAN
Voice Over WLAN (WLAN, LAN, IEEE 80211), "VoWLAN"
...
@item VPD
Virtual Product Development (CAD)
...
@item VQEG
Video Quality Experts Group (video)

@item VQM
Visual Quality Metric (video)
...
@item VRM
Voltage Regulator Modul (Iwill)

@item VRML
Virtual Reality Modeling Language (VR, ISO/IEC 14772, W3C)

@item VROC
Virtual-RAID-On-CPU (RAID, CPU, ASUS)

@item VROOMM
Virtual Realtime Object Oriented Memory Manager (OOP)

@item VRR
Variable Refresh Rate (HDMI)

@item VRRP
Virtual Router Redundancy Protocol (RFC 2338/3768)
...
@item VSA
Visual Studio for Applications (MS)
...
@item VSAN
Virtual Storage Area Network (SAN, VMware)

@item VSAT
Very Small Aperture Terminal

@item VSC
VMware [GSX] Server Console (VMware)
...
@item VSGA
Virtual Shared Graphics Acceleration (VMware, vDGA), "vSGA"
...
@item VSS
Volume Shadow copy Service (MS, Windows)

@item VST
VermittlungsSTelle (DTAG), "VSt"

@item VST
Virtual Studio Technology (Steinberg, audio)

@item VSTF
Volunteer Spam Task Force (Wikia, CVN)

@item VSTI
Virtual Studio Technology Instrument (VST), "VSTi"

@item VSTO
Visual Studio TOols for the microsoft office system (MS)

@item VSTS
Visual Studio Team Services (MS, TFS)

@item VSTS
Visual Studio Team System (MS)

@item VSU
Vertex/Scalability Unit (3Dlabs)
...
@item VTKE
Verbund der TeleKommunikations-Endgeraetehersteller (org.)

@item VTL
Velocity Template Language (Apache, Java)
...
@item VTSAPI
Voucher Trading System Application Programming Interface (API, RFC 4154), "VTS-API"
...
@item VVC
Versatile Video Codec (MPEG)

@item VVIDD
VESA Video Interface for Digital Displays (VESA, LCD)

@item VXAO
VoXel Ambient Occlusion (Nvidia)
