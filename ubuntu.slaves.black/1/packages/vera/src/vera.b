...
@item BARB
Bluetooth Architecture Review Board (org., Bluetooth)
...
@item BATV
Bounce Address Tag Validation (SMTP)
...
@item BBIAS
Bye, Back In A Second (slang, Usenet, IRC)
...
@item BCI
ByteCode Interpreter (TrueType, OpenType)
...
@item BDD
Business Desktop Deployment (MS, Windows, Vista)
...
@item BDJ
Blu-ray Disc Java (BD, Java, J2ME), "BD-J"
...
@item BDPA
Blu-ray Disk Pure Audio (Universal, BD, audio)
...
@item BDRE
Blu-ray Disk - REwritable (BD), "BD-RE"
...
@item BDSL
Boundary Scan Description Language (JTAG, VHDL, IEEE 1149.1-2001)

@item BDTI
Berkeley Design Technology, Inc. (manufacturer), "BDTi"
...
@item BEREC
Body of European Regulators for Electronic Communications (org., Europe)
...
@item BFDI
Bundesbeauftragter Fuer den Datenschutz und die Informationsfreiheit Germany, "BfDI"
...
@item BIGSTAF
Breitbandiges Integriertes GefechtsSTAndFernmeldenetz (Bundeswehr)
...
@item BIKA
Barrierefreie Informations- und Kommunikationstechnologie fuer Alle (FIT)
...
@item BIU
Bundesverband Interaktive Unterhaltungssoftware [e.V.] Org., Germany
...
@item BLAS
Basic Linear Algebra Subroutines (Intel, API)
...
@item BLE
Bluetooth 4.0 Low Energy (Bluetooth)
...
@item BMC
Baseboard Management Controller (IPMI, BMC)
...
@item BOCA
Business Object Component Architecture (CORBA, OMG)
...
@item BPEL4WS
Business Process Execution Language for Web Services (XML, XSD, SOAP, WSDL, OASIS)
...
@item BPOS
Business Productivity Online Suite (MS)
...
@item BRIL
Bus Read Invalidate Line (Intel, CPU, BWIL)

@item BRIM
Bridge/Router Interface Module

@item BRL
Bus Read Line (Intel, CPU, BWL)

@item BRMS
Backup Recovery and Media Services (IBM)
...
@item BSR
Boundary-Scan-Register (JTAG, TAP, IC)
...
@item BTC
Block Turbo Codes (WIMAX, 802.16e)
...
@item BTH
Base Transport Header (Infiniband)

@item BTI
Branch Target Injection (CPU)
...
@item BTLE
BlueTooth Low Energy (Bluetooth), "BT LE"
...
@item BTNS
Better-Than-Nothing Security (IETF, WG)
...
@item BTRFS
BTRee Files System (Linux, FS), "btrfs"

@item BTRON
Business TRON (TRON)

@item BTS
Base Transceiver Station entities (BCF, BS, GSM, GPRS, UMTS, mobile-systems)
...
@item BTX
Balanced Technology eXtended [form factor] (AT, ATX)

@item BTX
BildschirmTeXt (network, Datex-J, DTAG), "Btx"

@item BTXVST
BildschirmTeXt-VermittlungsSTelle (BTX), "Btx-VSt"

@item BUGLAS
BUndesverband GLASfaseranschluss e.v. (org.)

@item BUI
Bus Interface Unit (DEC)

@item BUP
Buffer Underrun Protection (DVD, CD)
...
@item BVDW
BundesVerband Digitale Wirtschaft [e.V.] (org., Germany)

@item BVIT
BundesVerband InformationsTechnologien [e.v.] (org.)

@item BVSI
Berufsverband de Selbststaendigen in der Informatik [e.V.] Org., Germany

@item BW
Business Warehouse (DWH)

@item BWAM
Because We Are Mean (slang, Cygwin)
...
@item BWF
Broadcast Wave Format (DCI)

@item BWIL
Bus Write Invalidate Line (Intel, CPU, BRIL)

@item BWL
Bus Write Line (IIntel, CPU, BRL)
...
@item BZR
Bit Zone Recording (ROD, BIT)

@item BZR
BundesZentralRegister (IC)

@item BZT
Bundesamt fuer Zulassungen in der Telekommunikation (org., DTAG)

@item BZUE
Beleglose ZahlscheinUEberweisung (banking)
