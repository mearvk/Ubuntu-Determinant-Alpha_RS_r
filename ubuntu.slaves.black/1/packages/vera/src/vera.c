...
@item C2DM
Cloud to Device Messaging (Google)
...
@item CAA
Certification Authority Authorization (DNS, RFC 6844)
...
@item CABC
Content Adaptive Brightness Control (Dell)
...
@item CAIP
Computer Analysis of Images and Patterns (Konferenz)
...
@item CAL
Conversational Algebraic Language (JOSS)
...
@item CAML
Categorical Abstract Machine Language (INRIA), "Caml"

@item CAML
Collaborative Application Markup Language (MS, XML)
...
@item CAPI
Coherent Accelerator Processor Interface (NVidia)
...
@item CATI
Computer Aided / Assisted Telephone Interviewing
...
@item CBB
Current Branch for Business (MS, Windows)
...
@item CBEFF
Common Biometric Exchange Format Framework (NIST, ISO 19785)
...
@item CBS
Component Based Setup (MS, Windows)
...
@item CBT
Cursor Backward Tabulation (CFT)

@item CBU
Capacity BackUp (IBM, zSeries)
...
@item CCAP
Converged Cable Access Platform (DOCSIS)
...
@item CCDCOE
Cooperative Cyber Defence Centre of Excellence (org., NATO)
...
@item CCIA
Computer & Communication Industry Association (org.)
...
@item CDC
Communication Device Class (USB, MS, Windows, WMCDC)
...
@item CDD
Compatibility Definition Document (Android)
...
@item CDE
Common Desktop Environment (Unix)
...
@item CDM
Conceptional Data Model (DB, PDM, Powerdesigner)

@item CDM
Content Decryption Module (cryptography)
...
@item CDNC
Chinese Domain Name Consortium (org., Internet)
...
@item CDT
Cambridge Display Technology (manufacturer)
...
@item CEA
Capability-Exchange-Answer (CER, Diameter)
...
@item CEC
Consumer Electronics Control [protocol] (HDMI)
...
@item CEDA
CICS Enhanced resource Definition online Application (CICS, IBM)
...
@item CEIP
Customer Experience Improvement Program (MS)
...
@item CEMT
CICS Enhanced Master Terminal transaction (CICS, IBM)
...
@item CENTR
Council of European National Top level Domain Registries (org., TLD)

@item CEPAC
CMOS-Ein-Platinen-Allzweck-Computer (IC, CMOS, C'T)

@item CEPD
Comite Europeen de la Protection des Donnees (org.)
...
@item CER
Capability-Exchange-Request (CEA, Diameter)
...
@item CGE
Carrier Grade Edition (Linux, MontaVista, CGL)

@item CGEMM
Complex single precision GEneral Matrix Multiply (GEMM, BLAS)
...
@item CGMSA
Content Generation Management System - Analog (WPE), "CGMS-A"
...
@item CIIRC
China Internet Illegal Information Reporting Center (China, org.)
...
@item CISA
Cybersecurity and Infrastructure Security Agency (org.)
...
@item CISRAM
Cybernetics & Intelligent Systems - Robotics, Automation & Mechatronics (IEEE, conference), "CIS-RAM"

@item CISRT
Chinese Internet Security Response Team (org., China)
...
@item CIU
Customer Initiated Upgrade (IBM, zSeries)

@item CIU
Customer-Initiated Upgrade [switch] ??? (IBM)
...
@item CKRM
Class-based Kernel Resource Manager / Management
...
@item CLDC
Connected Limited Device Configuration (KVM, CDC, Java, JSR-139)

@item CLDR
Common Locale Data Repository (Android)
...
@item CLP
Cumulative Licensing Program (Adobe)
...
@item CLX
Cascade Lake Xeon (Intel, CPU)

@item CLX
Class Library for cross platform (Delphi, Windows, Linux)

@item CLXAP
Cascade Lake Xeon - Advanced Performance (Intel,CPU), "CLX-AP"
...
@item CMTC
Critical Machine-Type Communication (MTC, 3GPP), "C-MTC"

@item CMTS
Cable MODEM Termination System (DOCSIS, MODEM)
...
@item CNIL
Commission Nationale de l'Informatique et des Libertes (org., France)
...
@item COACH
Component based Open source ArCHitecture for distributed systems (CORBA)
...
@item COFEE
Computer Online Forensic Evidence Extractor (MS)
...
@item COLLADA
COLLAborative Design Activity (XML, 3D)
...
@item COPP
Certified Output Protection Protocol (MS, RSA, WPE)

@item COPPA
Children's Online Privacy Protection Act (Internet)
...
@item CPE
Common Platform Enumeration (URI, NIST)
...
@item CPPM
Content Protection for Pre-recorded Media (CPRM)

@item CPPS
Card / Paper tape Programming System (OS, IBM)

@item CPR
CPU Parameter Recall (Asus), "C.P.R."

@item CPRM
Content Protection for Recordable Media (CPPM)
...
@item CPTA
China Power Technology Alliance (org.)
...
@item CRESTA
Collaborative Research into Exascale Systemware, Tools and Applications (Europe)
...
@item CRS
CMOS Reload Switch (CMOS, Gigabyte), "C.R.S."
...
@item CSAIL
Computer Science and Artificial Intelligence Labs (MIT)
...
@item CSFB
Circuit Switched FallBack (LTE, UMTS, GSM)
...
@item CSI
Code Sequence Identification [technology] (Lavasoft, Ad-Aware)

@item CSI
Common Secure Interoperability (CORBA)
...
@item CSIP
Chipset Stable Image Program

@item CSIP
Commercial Stable Image Platform (AMD)
...
@item CSM
Compatibility Support Module (UEFI)
...
@item CSRC
Computing Science Research Center (org., Berkeley, BSD)

@item CSRF
Cross Site Request Forgery (WWW, Internet)

@item CSRG
Computer Systems Research Group (org., Unix, BSD)
...
@item CTA
Consumer Technology Association (org.)

@item CTAN
Comprehensive Tex Archive Network (TeX, FTP), pause
...
@item CTC
Convolutional Turbo Coding (WIMAX, 802.16e)
...
@item CUL
see you Later (slang, Usenet, IRC)
...
@item CVN
CounterVandalism Network (Wikia, VSTF)
