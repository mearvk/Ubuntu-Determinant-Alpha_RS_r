...
@item TADF
Thermally Activated Delayed Fluorescense (OLED)
...
@item TAE
Telekommunikations-AnschlussEinheit (DTAG)
...
@item TASE
Telecontrol Application Service Element (ICCP)
...
@item TAUCIS
Technikfolgenabschaetzung - Ubiquitaeres Computing und Informationelle Selbstbestimmung (BMBF)
...
@item TBI
Total Business Integration [system]
...
@item TCA
Target Channel Adapter (Infiniband)

@item TCA
Typo3 Configuration Array (TYPO3)
...
@item TCAM
Ternary Content Addressable Memory (CAM)

@item TCAP
Transaction Capabilities Applications Part (MSC, GSM, IN, mobile-systems, SS7)

@item TCB
Task Control Block (BS2000)

@item TCB
Trusted Computing Base (MS, Palladium)

@item TCC
Telecommunications Center

@item TCE
Typo3 Core Engine (TYPO3)

@item TCF
Thin Client Framework (IBM, Java)
...
@item TCG
Trusted Computing Group (org., DRM, AMD, HP, IBM, Intel, MS, ...)
...
@item TCK
Technology Compatibility Kit (JCP, Java, Sun)
...
@item TCPA
Trusted Computing Platform Alliance (org., cryptography, DRM, TCPA)
...
@item TCQ
Tagged Command Queueing (Linux, SCSI)

@item TCRTP
Tunneling multiplexed Compressed Real Time Protocol (RTP, RFC 4170)

@item TCS
Transmission Convergence Sublayer (ATM)

@item TCSEC
Trusted Computer System Evaluation Criteria (Orange Book, NCSC, USA)
...
@item TDL
Trusted Device List (DCI)

@item TDLS
Tunneled Direct Link Setup (WLAN)

@item TDM
Telekom Designed Networks (DTAG)
...
@item TDMB
Terrestrial Digital Multimedia Broadcasting (DMB), "T-DMB"

@item TDMOIP
Time Division - Multiplex Over Internet Protocol (IP, DSL), "TD-MoIP"
...
@item TDOP
Time Dilution Of Precision (GPS)
...
@item TDP
Thermal Design Power (CPU, ACP, SDP)
...
@item TDSCDMA
Time Division Synchronous Code Division Multiple Access (mobile-systems, CDMA)

@item TDSL
Telekom Digital Subscriber Line (DTAG, Germany), "T-DSL"

@item TDSV
Telekommunikation-DatenSchutzVerordnung (BMWI)

@item TDT
Time and Date Table (DVB, UTC)
...
@item TEBI
TEraBInary [(2^10)^4] (IEC)

@item TEC
Text Encoding Converter (Apple)

@item TEC
Trace Execution Cache (Intel, Pentium, CPU)
...
@item TFLOPS
Tera FLoating-point Operations Per Second (CPU, GPU)
...
@item TFRP
TCP Friendly Rate Control (TCP, RFC 3448)

@item TFS
Team Foundation Server (MS, ALM)
...
@item TGMS
TransGear Media Server (Transgear)

@item THAMA
Trident Hardware-Assisted MPEG-2/AC-3 (Trident, MPEG, DVD, AC-3)

@item THATIC
Tianjin Haiguang Advanced Technology Investment Corporation (manufacturer)
...
@item THUMS
Total HUman Model for Safety (Toyota, VPD)
...
@item TIA
Telecommunications Industries Association (org.)
...
@item TIDSE
Techniken fuer Interaktives Storytelling und Entertainment (conference, ZGDV)
...
@item TIMTOWTDI
There Is More Than One Way To Do It (PERL, slang)
...
@item TIS
Tools Interface Standard [committee] (org., IBM, Lotus, MS, Watcom, SCO, Unix)

@item TIS
TPM Interface Specification (TPM)

@item TISN
Tokyo International Science Network (network)

@item TITN
Traitement de l'Information-Techniques Nouvelles (manufacturer, France)
...
@item TKIP
Temporal Key Integrity Protocol (WEP, WLAN, WPA)

@item TKO
TeleKommunikationsOrdnung

@item TKUEV
Telekommunikations-UEberwachungsverordnung Germany

@item TKV
TeleKommunikationsVerordnung

@item TL
Telecommunication Leadership [standard] (QuEST)
...
@item TLDP
The Linux Documentation Project (Linux)
...
@item TLP
Transactional Licensing Program (Adobe)
...
@item TLS
Transport Layer Security [protocol] (SSL, TCP, RFC 2246)
...
@item TMDS
Transition Minimized Differential Signaling (VESA, LCD, DVI)
...
@item TMSS
[LambdaManager] Terabit MultiService Switch (Lucent, LambdaXtreme, BIT)
...
@item TMTP
TrackMania Transfer Protocol (HTTP, WWW)

@item TMU
Texture Mapping Unit (GPU)
...
@item TNMOC
The National Museum Of Computing (org., UK)
...
@item TOSMAPDS
TOShiba Microprocessor Aided Power system control - DynaStream (DCS, Toshiba), "TOSMAP-DS"
...
@item TPC
The PERL Conference (PERL)

@item TPC
Transaction Processing Council (org., manufacturer, DB)

@item TPC
Transmit Power Control (HiperLAN/2, WLAN)

@item TPCC
Third Party Call Control

@item TPCC
TPC benchmark C (TPC), "TPC-C"

@item TPCE
TPC benchmark E (TPC), "TPC-E"

@item TPCH
TPC benchmark H (TPC), "TPC-H"

@item TPCW
TPC benchmark W (TPC, WWW), "TPC-W"
...
@item TPR
Task Priority Register (IC)

@item TPRM
Transaction Programmer's Reference Manual (Sun)
...
@item TQ
Tagged Queuing (SCSI, HDD)

@item TR
Task Register (CPU, Intel, assembler, IC)

@item TR
Technical Report (DSL-Forum, WT)

@item TR
Technology Refresh (IBM)

@item TRAP
Tandem Recursive Algorithm Process

@item TREC
Text REtrieval Conference (Konferenz)
...
@item TRR
Trusted Recursive Resolver (DNS)

@item TRT
The Right Thing (slang, Usenet, IRC)
...
@item TSC
Time Stamp Counter (IC, Intel, Pentium)

@item TSC
Triton System Controller (Intel, Triton, IC)

@item TSCT
Tangan Silence Control Technology (Tangan)

@item TSD
Ton-Saettigung-Dunkelstufe (color system)
...
@item TSF
TOE Security Functions (CC, TOE)

@item TSFE
TypoScript Front End (TYPO3)
...
@item TSM
Tivoli Storage Manager (Tivoli, IBM)

@item TSM
Topology Specific Module (ODI)

@item TSM
Trusted Service Manager (mobile-systems, TSP)
...
@item TSOE
Time Sharing Option/Extensions (IBM), "TSO/E"
...
@item TSP
Trusted Service Provider (mobile-systems, TSM)
...
@item TTCAN
Time Triggered Communication on Controller Area Network (CAN)
...
@item TTKC
The Thursday Killers Clan (user group)
...
@item TTLS
Tunneled Transport Layer Security [protocol] (TLS, SSL, WLAN)
...
@item TTW
Through The Web (WWW, ZMI, ZOPE)

@item TTY
TeleTYpe

@item TTYL
Talk To You Later (slang, Usenet, IRC)
...
@item TUCAA
The Ultimate Computer Acronyms Archive (Internet, WWW)
...
@item TUHS
The Unix Heritage Society (org., Unix)
...
@item TVST
TeilnehmerVermittlungsSTelle (DTAG), "TVSt"
...
@item TXGA
Tesselar XGA [1920x1400] (XGA)

@item TXT
Trusted eXecution Technology (AMT)
