...
@item DAA
Digest Access Authentication (HTTP, RFC 2617)

@item DAAL
Data Analytic Acceleration Library (Intel)
...
@item DACS
Distributed Access Control System (Apache)
...
@item DADIU
Danske Akademi for Digital Interaktiv Underholdning (org., Denmark)
...
@item DAIN
Depth-Aware video frame Interpolation (AI)
...
@item DANE
DNS-based Authentication of Named Entities (DNS, DNSSEC, cryptography)
...
@item DARSIMCO
DARtmouth SINplified COde, "Darsimco"
...
@item DASP
Disk Anti Shock Protection (Acer)
...
@item DAU
Digital Annealing Unit (Fujitsu, ASIC)

@item DAU
Duemmster Anzunehmender User (slang, Usenet, IRC)

@item DAV
[WWW] Distributed Authoring Versioning (WWW, HTTP)
...
@item DCBX
Data Center Bridging eXchange (LLDP, DCB)
...
@item DCCA
Dependable Computing for Critical Applications (Konferenz)
...
@item DCHSDPA
Dual Cell High Speed Downlink Packet Access (HSDPA, mobile-systems), "DC-HSDPA"
...
@item DCO
Device Configuration Overlay (ATA)
...
@item DCTCP
Data Center Transmission Control Protocol (TCP), "DP TCP"
...
@item DDA
Dynamic Data Authentication (EMV, cryptography)
...
@item DDTM
Data Dedicated Transmission Mode (CDMA)
...
@item DECREE
DARPA Experimental Cybersecurity Research Evaluation Environment (DARPA, OS)

@item DECS
Domino Enterprise Connection Services (Lotus)

@item DECT
Digital Enhanced Cordless Telecommunications (telecommunication)

@item DECT
Digital European Cordless Telecommunications (veraltet) (telecommunication)

@item DECTULE
DECT Ultra Low Energy (DECT), "DECT ULE"
...
@item DFS
Deniable File System (FS)

@item DFS
Direct File System (Novell, Oracle)

@item DFS
Disk Filing System (Acorn, FS)
...
@item DGEMM
Double precision GEneral Matrix Multiply (GEMM, BLAS)
...
@item DHES
Direct Heat Exhaust System

@item DHIS
Distributed Heterogeneous Information Systems

@item DHT
Distribution Hash Table (P2P)

@item DHTML
Dynamic HyperText Markup Language (HTML)

@item DHWG
Digital Home Working Group (org.)
...
@item DIAS
Digital Information Archiving System (IBM, Netherlands)
...
@item DIDW
Deutschland Im Deep Web (Internet), "DiDW"
...
@item DIF
Directory Interoperability Forum (org., IBM, Lotus, Novell, Oracle, ...)
...
@item DILA
Digital Direct Drive Image Light Amplifier (JVC, CMOS, LCOS, HDTV), "D-ILA"
...
@item DIO
Direct Input Output (I/O, HDD, SCSI)

@item DIP
Density-Independent Pixel (DP, SIP, SP)
...
@item DITA
Darwin Information Typing Architecture (XML, IBM, OASIS)

@item DITCO
Defense Information Technology Contracting Office (org., mil., DISA, USA)

@item DITSCAP
Department of defense Information Technology Security Certification and Accreditation Process (DIACAP)

@item DITU
Data Interception Technology Unit
...
@item DKE
Deutsche Kommission Elektrotechnik, elektronik, informationstechnik (org., DIN, VDE)

@item DKIM
Domain Keys Identified Mail (Domain, RFC 6376)

@item DKMS
Dynamic Kernel Module Support (Linux)
...
@item DLJ
Distro License for Java (JRE, Linux)
...
@item DLM
Distributed Lock Manager (Linux, RedHat, GFS)
...
@item DMAP
Digital Media Access Protocol (Apple)

@item DMARC
Domain-based Message Authentication, Reporting & Conformance (SPF, DKIM, RFC 7489)

@item DMB
Digital Multimedia Broadcasting (DTAG, Blaupunkt, DAB, MPEG)
...
@item DMP
Designated Mailers Protocol (SPAM)
...
@item DMS
Data Management System (ESA, Venus-Express)
...
@item DMX
Distributed Multihead X (X-Windows)

@item DMZ
DeMilitarized Zone (LAN, Internet)

@item DN
Distinguished Name (X.500, LDAP)
...
@item DNI
Digital New Initiative (Google, Europe)
...
@item DNSRR
Domain Name Service Round Robbing (DNS)

@item DNSTAN
Digitale NebenSTellenANlagen (DTAG), "DNStAn"

@item DNUG
Deutsche Notes User Group [e.v.] (org., user group, Lotus)
...
@item DOAG
Deutsche Oracle AnwenderGruppe [e.V.] (Oracle, user group, org.)

@item DOAM
Distributed Office Applications Model (ISO, IEC, DIS 10031-1 f.)

@item DOAP
Description of a Project (XML)
...
@item DOCSISOSS
DOCSIS Operations Suport Services (DOCSIS), "DOCSIS-OSS"
...
@item DOMAIN
Distributed Operating Multi Access Interactive Network (Apollo, DOMAIN)
...
@item DOT
DNS Over TLS (DNS, TLS, RFC 7858), "DoT"
...
@item DP
Density-independent Pixel (DIP, SIP, SP)
...
@item DPBO
Downstream Power Back Off (DSLAM)
...
@item DPC
Deferred Procedure Call (MS, Windows)

@item DPC
DIMM Per Channel (DIMM)
...
@item DPCM
Differential Pulse Code Modulation (PCM)

@item DPCX
Distributed Processing Control eXecutive (IBM)

@item DPD
Dead Peer Detection (Internet, Firewall)
...
@item DPF
Discrete Packet Format (Exabyte, VXA, Streamer)
...
@item DPI
Deep Packet Inspection (TCP/IP, UDP)
...
@item DPM
Dynamic Power Management (AMD, GPU)
...
@item DPOE
DOCSIS Provisioning Over EPON (DOCSIS, EPON), "DPoE"

@item DPOF
Digital / Direct Print Order Format (printing)
...
@item DPWS
Devices Profile for Web Services (WWW)
...
@item DQP
Distributed Query Processing (OGSA-DAI, grid)
...
@item DRAC
Dell Remote Access Card (Dell, BMC)
...
@item DRP
Dynamic Resource Pool (Solaris, Sun)

@item DRPM
Dynamic Rotations Per Minute (HDD)
...
@item DRS
Directory Replication Service (MS, AD)

@item DRS
Double-Ring-with-Star [topology]
...
@item DS1E1
Digital Signal level 1/European 1 (RFC 1406), "DS1/E1"
...
@item DS3E3
Digital Signal level 3/European 3 (RFC 1407), "DS3/E3"
...
@item DSAG
Deutsche SAP AnwenderGruppe (SAP, user group)
...
@item DSC
DECT Standard Cipher (DECT, cryptography)

@item DSC
Desired State Configuration (MS, Windows)

@item DSC
Desired State Configuration (MS, Windows)

@item DSC
Display Stream Compression (VESA)
...
@item DSD
Direct Stream Digital (Sony, Philips, SACD)
...
@item DSGVO
DatenSchutz-GrundVerOrdnung Germany
...
@item DSL
Dataset and Snapshot Layer (ZFS)
...
@item DSM
Digital Standard MUMPS (MUMPS, DEC)
...
@item DSMCC
Digital Media Storage - Command and Control (DVB), "DSM-CC"
...
@item DSRH
Double-Sided RowHammer (DRAM, SSRH)
...
@item DTAZV
DatenTraegeraustausch, belegloser AuslandsZahlungsVerkehr (banking)
...
@item DTCSA
Digital Transition Content Security Act (USA)
...
@item DTE
Domain and Type Enforcement (TE)
...
@item DTI
Document Technologies, Inc. (company, USA)
...
@item DTML
Document Template Markup Language (ZOPE)
...
@item DTO
DeviceTree Overlay (CAN)

@item DTOS
Distributed Trusted Operating System (OS, DTE)
...
@item DTS
Digital Thermal Sensor (Intel, IC, CPU)
...
@item DTSES
Digital Theatre Sound - Extended Surround (audio), "DTS-ES"

@item DTSS
Dartmouth Time-Sharing System (OS)

@item DTT
Dual Transformer Technology (Tangan)

@item DTTS
Deutsche Telekom Technischer Service (org.)
...
@item DUNDI
Distributed Universal Number DIscovery (VoiP), "DUNDi"
...
@item DVA
Data Virtual Address (ZFS)

@item DVA
DatenVerarbeitungsAnlage

@item DVA
Dynamic Vibration Absorber (CD, DVD, Samsung)
...
@item DVBH
Digital Video Broadcasting - Handheld (DVB), "DVB-H"
...
@item DVCS
Distributed / Decentralized Version Control System (DRCS)
...
@item DVI
Digital Video Interactive [old term] (LCD, Intel, IBM, Lotus, HP, Compaq, ...)

@item DVI
Digital Visual Interface (DVD, video)

@item DVID
Digital Video Interactive - Digital (LCD), "DVI-D"

@item DVID
Digital Video Interactive - Integrated (LCD), "DVI-I"

@item DVID
Dynamic - Voltage IDentification (Intel, CPU), "D-VID"
...
@item DWA
Device-Watchdog-Answer (DWR, Diameter)
...
@item DWR
Device-Watchdog-Request (DWA, Diameter)

@item DWS
Delphi Web Script (Delphi, Borland, WWW)

@item DWS
Digital World Services (ISP)
