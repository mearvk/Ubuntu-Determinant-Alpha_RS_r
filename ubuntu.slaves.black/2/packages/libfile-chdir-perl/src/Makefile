# This Makefile is for the File::chdir extension to perl.
#
...
#     ABSTRACT => q[a more sensible way to change directories]
#     AUTHOR => [q[David A Golden <dagolden@cpan.org>, Michael G Schwern <schwern@pobox.com> (original author)]]
#     BUILD_REQUIRES => { warnings=>q[0], Test::More=>q[0], File::Find=>q[0], File::Temp=>q[0] }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[6.30] }
#     DISTNAME => q[File-chdir]
#     EXE_FILES => []
#     LICENSE => q[perl]
#     NAME => q[File::chdir]
#     PREREQ_PM => { warnings=>q[0], vars=>q[0], Exporter=>q[0], Carp=>q[0], strict=>q[0], Test::More=>q[0], File::Find=>q[0], File::Temp=>q[0], File::Spec::Functions=>q[3.27], Cwd=>q[3.16] }
#     VERSION => q[0.1008]
#     test => { TESTS=>q[t/*.t] }
...
TO_INST_PM = lib/File/chdir.pm

PM_TO_BLIB = lib/File/chdir.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/File/chdir.pm $(INST_MAN3DIR)/File::chdir.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               File-chdir' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.1008' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           a more sensible way to change directories' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - David A Golden <dagolden@cpan.org>, Michael G Schwern <schwern@pobox.com> (original author)' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Cwd:                  3.16' >> META_new.yml
	$(NOECHO) $(ECHO) '    Exporter:             0' >> META_new.yml
	$(NOECHO) $(ECHO) '    File::Spec::Functions:  3.27' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.1008">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>a more sensible way to change directories</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>David A Golden &lt;dagolden@cpan.org&gt;, Michael G Schwern &lt;schwern@pobox.com&gt; (original author)</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Carp::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Cwd::" VERSION="3.16" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Exporter::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Spec::Functions" VERSION="3.27" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/File/chdir.pm blib/lib/File/chdir.pm 
