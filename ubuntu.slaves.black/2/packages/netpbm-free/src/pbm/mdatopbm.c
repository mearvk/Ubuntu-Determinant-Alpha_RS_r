...
static mdbyte getbyte()		/* Read a byte from the input stream, */
{				/* with error trapping */
	int b;
...
static void render_byte(int *col, int *xp, int *yp, int b)
{
	int mask = 0x80;
	int n;
	int y3 = *yp;
...
static void md2_trans()		/* Convert a MicroDesign 2 area to PBM */
{				/* MD2 has RLE encoding that may go over */
	...
	while (y1 < nInRows)
	{
		...
		if (b == 0 || b == 0xFF)	/* RLE sequence */
		{
			c = getbyte();
			if (c == 0) c = 256;
			while (c > 0) { render_byte(&col, &x1, &y1, b);	--c; }
		}
		else render_byte(&col, &x1, &y1, b);	/* Not RLE */
...
static void md3_trans()	/* Convert MD3 file. MD3 are encoded as rows, and */
...
{		 
	printf("mdatopbm v1.00, Copyright (C) 1999 John Elliott <jce@seasip.demon.co.uk>\n"
...
int main(int argc, char **argv)
{
	...
	if (!data || !mdrow)
	{
		if (data) pbm_freearray(data, nOutRows);
		if (mdrow) free(mdrow);
		pm_error("Not enough memory for conversion.\n");
	}
	if (header[21] == '0') md2_trans();
	else		       md3_trans();
