...
#define Atthick 1.03              /* Atmosphere thickness as a percentage
    ...
    if (!stars) {
        u = (double *) malloc2(screenxsize, sizeof(double));
        u1 = (double *) malloc2(screenxsize, sizeof(double));
        bxf = (unsigned int *) malloc2(screenxsize,
                                      sizeof(unsigned int));
        bxc = (unsigned int *) malloc2(screenxsize,
                                      sizeof(unsigned int));
        ...
        if (clouds) {
            ...
            for (j = 0; j < screenxsize; j++) {
                ...
                */
                ...
                w = (r > 127.0) ? (RGBQuant * ((r - 127.0) / 128.0)) : 0;
