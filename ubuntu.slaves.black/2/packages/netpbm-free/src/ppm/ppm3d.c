...
main (int argc, char *argv[]) {
   ...
   pixval Rmaxval;
    ...
    if (argc-1 > 3 || argc-1 < 2) 
        pm_usage( "leftppmfile rightppmfile [horizontal_offset]" );
    ...
    for ( row = 0; row < rows; ++row ) {
        ...
        if (maxval <= 255) {
            ...
            for (col = 0; col < cols; ++col) {
                Lgrayrow[col] = (gray) 
                    ( ( times77[PPM_GETR( Lpixelrow[col] )] + 
                        times150[PPM_GETG( Lpixelrow[col] )] +
                        times29[PPM_GETB( Lpixelrow[col] )] ) >> 8 );
                Rgrayrow[col] = (gray) ( 
                    ( times77[PPM_GETR( Rpixelrow[col] )] + 
                      times150[PPM_GETG( Rpixelrow[col] )] +
                      times29[PPM_GETB( Rpixelrow[col] )] ) >> 8 );
            } 
        } else {
            int col;
            /* Can't use fast approximation, so fall back on floats. */
            for (col = 0; col < cols; ++col) {
                Lgrayrow[col] = (gray) ( PPM_LUMIN( Lpixelrow[col] ) + 0.5 );
                Rgrayrow[col] = (gray) ( PPM_LUMIN( Rpixelrow[col] ) + 0.5 );
            } 
        }
        {
            int col;
