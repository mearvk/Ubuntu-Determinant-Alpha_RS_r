...
main( argc, argv )
	...
	switch ( PNM_FORMAT_TYPE(format) )
	    ...
	    chv = ppm_computecolorhist( (xel **) xels, cols, rows, xmaxval+1, 
                                    &colors );
		...
		pm_message(
	      "dump file.  If you want PseudoColor, try doing a 'ppmquant %ld'.",
		    xmaxval );
		direct = 1;
		}
	    else
	...
	if ( direct )
		...
		for ( col = 0; col < cols; ++col )
		    ...
		    ul = ( ( PPM_GETR( xels[row][col] ) * xmaxval / lmaxval ) << 16 ) |
			 ( ( PPM_GETG( xels[row][col] ) * xmaxval / lmaxval ) << 8 ) |
			 ( PPM_GETB( xels[row][col] ) * xmaxval / lmaxval );
	    ...
	    {
	    for ( col = 0; col < cols; ++col)
		putchar( ppm_lookupcolor( cht, &xels[row][col] ) );
	    }
