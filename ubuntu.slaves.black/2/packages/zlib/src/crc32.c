...
extern unsigned int crc32_le_vgfm_16(unsigned int, const unsigned char *, z_size_t);

local uint32_t s390_crc32_vx(uint32_t crc, const unsigned char *buf, uint64_t len) {
...
#ifdef __s390__
/* hwcap is passed to ifuncs on s390 so we can avoid a call to getauxval
   while relocations are still being processed */
				unsigned long hwcap
#else
				void
#endif
    ...
    z_size_t len;
{
    ...
    if (!crc32_func)
#ifdef __s390__
        crc32_func = crc32_z_ifunc(getauxval(AT_HWCAP));
#else
...
/* ========================================================================= */
ZLIB_INTERNAL unsigned long crc32_big(crc, buf, len)
    unsigned long crc;
    const unsigned char FAR *buf;
    z_size_t len;
{
