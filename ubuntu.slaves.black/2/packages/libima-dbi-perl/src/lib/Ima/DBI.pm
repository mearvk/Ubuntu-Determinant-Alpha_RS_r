...
SQL information with your class (instead of as separate objects).
...
That's what Ima::DBI enforces.  Extremely handy in persistent environments
...
Ima::DBI's caching might prove to be an unnecessary performance hog if
...
to retrieve database handles ($dbh).
