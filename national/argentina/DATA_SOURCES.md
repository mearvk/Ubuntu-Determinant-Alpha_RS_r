# Argentina Data Sources and Full-Data Build
...
- **World Bank — World Development Indicators (WDI).** Macro/structural series for Argentina
  (`ARG`): GDP, GDP growth, agriculture value added, agricultural/arable land, employment.
  - https://data.worldbank.org · API: https://api.worldbank.org/v2/country/ARG/indicator/{code}
...
- **INDEC (Instituto Nacional de Estadistica y Censos).** National statistical office; preferred for table-level supplementation
  where a compatible series is transcribed. https://www.indec.gob.ar/
...
`tools/arg` … see `tools/` builder for this country. It generates ten domain
