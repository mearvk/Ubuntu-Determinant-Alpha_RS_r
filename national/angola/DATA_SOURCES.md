# Angola Data Sources and Full-Data Build
...
- **World Bank — World Development Indicators (WDI).** Macro/structural series for Angola
  (`AGO`): GDP, GDP growth, agriculture value added, agricultural/arable land, employment.
  - https://data.worldbank.org · API: https://api.worldbank.org/v2/country/AGO/indicator/{code}
...
- **INE Angola (Instituto Nacional de Estatistica).** National statistical office; preferred for table-level supplementation
  where a compatible series is transcribed. https://www.ine.gov.ao/
...
`tools/ago` … see `tools/` builder for this country. It generates ten domain
