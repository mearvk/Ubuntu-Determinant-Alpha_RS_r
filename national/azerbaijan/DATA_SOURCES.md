# Azerbaijan Data Sources and Full-Data Build
...
- **World Bank — World Development Indicators (WDI).** Macro/structural series for Azerbaijan
  (`AZE`): GDP, GDP growth, agriculture value added, agricultural/arable land, employment.
  - https://data.worldbank.org · API: https://api.worldbank.org/v2/country/AZE/indicator/{code}
...
- **AZSTAT (State Statistical Committee).** National statistical office; preferred for table-level supplementation
  where a compatible series is transcribed. https://www.stat.gov.az/
...
`tools/aze` … see `tools/` builder for this country. It generates ten domain
...
3. Geography must be Azerbaijan.
