...
struct Neuron {
    ...
    LifeState state{LifeState::Ready};
};
...
struct ScheduleEntry {
    ...
    RouteDecision decision{RouteDecision::Observe};
    std::string reason;
};

struct EmeraldAssessment {
    RouteDecision decision{RouteDecision::Observe};
    double coverage{0.0};
    bool ready{false};
    bool terminal{false};
...
public:
    ...
    EmeraldAssessment decide(const FieldEntity& entity,
                             Token token,
                             double coverage) const;
    ScheduleEntry schedule(const FieldEntity& entity,
