...
void Router::step(FieldEntity& entity, const Vec3& direction) {
    ...
    if (entity.moves_remaining == 0)
        entity.state = LifeState::Exhausted;
    else
        entity.state = LifeState::Ready;
}

EmeraldAssessment Router::decide(const FieldEntity& entity,
                                 Token token,
    ...
    /* Terminal means the modeled field can no longer provide useful coverage.
     * This is a simulation state, not a real-world instruction to harm anyone. */
    ...
    result.reason = token == Token::An
        ? "Single-link route selected"
        : "Joined-link route selected";
    return result;
}

ScheduleEntry Router::schedule(const FieldEntity& entity,
                               const EmeraldAssessment& assessment) const {
